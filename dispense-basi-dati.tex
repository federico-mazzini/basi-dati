\documentclass[a4paper,12pt]{article}

% Lingua e codifica
\usepackage[italian]{babel}
\usepackage[utf8]{inputenc}
\usepackage[T1]{fontenc}
\usepackage{lmodern}
\usepackage{graphicx}

% Impaginazione e matematica
\usepackage{geometry}
\geometry{margin=2.5cm}
\usepackage{amsmath, amssymb}
\usepackage{booktabs}
\usepackage{array}
\usepackage{hyperref}
\usepackage{tikz}              % Pacchetto principale per i disegni
\usetikzlibrary{arrows.meta}   % Per frecce avanzate (tipo freccia piena o tratteggiata)
\usetikzlibrary{positioning}   % Per posizionare i nodi rispetto ad altri nodi
\usetikzlibrary{shapes}        % Per avere nodi con forme arrotondate, rettangoli, cerchi, ecc.
\usetikzlibrary{backgrounds,decorations.pathreplacing,calc}

% Comandi utili
\newcommand{\code}[1]{\texttt{#1}}

\begin{document}

\begin{titlepage}
\centering

{\Large I.I.S. \textit{``Francesco Alberghetti''} Imola}\\[3cm]

\rule{0.85\textwidth}{0.8pt}\\[0.9cm]

{\huge\bfseries BASI DI DATI}\\[0.4cm]
{\Large DBMS, Modello Relazionale e linguaggio SQL}\\[0.6cm]

\rule{0.85\textwidth}{0.8pt}\\[2.5cm]

{\Large Materiale didattico di Informatica}\\[0.3cm]


% ICONA OPZIONALE (consigliata)
% \includegraphics[width=0.3\textwidth]{images/cover_icon.png}

\vfill

{\large Prof. Federico Mazzini}\\[0.3cm]
{\large Anno scolastico 2025 -- 2026}

\end{titlepage}

\newpage
\tableofcontents
\newpage

\section{Introduzione}
Ogni applicazione informatica che gestisce informazioni persistenti --- dal registro elettronico ai social network, dai sistemi bancari alle piattaforme di streaming --- si basa su una o più basi di dati.

Questa disciplina non riguarda la programmazione, ma il \textbf{modo in cui rappresentiamo le informazioni, le gestiamo e le interroghiamo} all’interno di un sistema informatico.
%==============================================================================
\subsection{Dato e Informazione}
%==============================================================================

Per iniziare lo studio delle basi di dati, è fondamentale chiarire la differenza tra due concetti che spesso vengono confusi: il dato e l'informazione.

Un \textbf{dato} è una rappresentazione oggettiva e non interpretata della realtà. Può essere un numero, una stringa di testo, un simbolo o un'immagine. Di per sé, il dato non ha un significato completo perché manca di contesto. Ad esempio, se scriviamo il numero "8" su un foglio, questo è un dato puro, ma non sappiamo se indica un orario, un voto scolastico o una quantità.

L'\textbf{informazione}, invece, è ciò che otteniamo quando interpretiamo un dato all'interno di un contesto specifico. È il dato che arricchisce la nostra conoscenza. Tornando all'esempio precedente, se specifichiamo che "8" è il voto preso nella verifica di Informatica da uno studente specifico, abbiamo trasformato il dato in informazione.

Le basi di dati si occupano principalmente di memorizzare e gestire i dati, ma il loro obiettivo finale è fornire informazioni utili agli utenti.

%==============================================================================
\subsection{Sistemi Informativi e Sistemi Informatici}
%==============================================================================

Un \textbf{Sistema Informativo (SI)} è l’insieme organizzato di \textbf{informazioni, procedure e persone} che un’organizzazione utilizza per svolgere le proprie attività e supportare i processi decisionali.  
Un sistema informativo descrive quindi \textbf{che informazioni servono}, \textbf{come vengono raccolte} e \textbf{come vengono utilizzate}, indipendentemente dal fatto che vengano gestite con strumenti digitali o meno.

È importante sottolineare che un sistema informativo \textbf{esiste anche senza computer}: un archivio cartaceo, un registro, o una serie di procedure manuali costituiscono comunque un sistema informativo, perché permettono di gestire informazioni a supporto dei processi dell’organizzazione.

Ad esempio, in un’azienda che produce scarpe, il sistema informativo serve a gestire informazioni su clienti, prodotti, fornitori, dipendenti e vendite. Queste informazioni sono utilizzate quotidianamente per svolgere le attività operative e per prendere decisioni (cosa produrre, cosa riordinare, cosa vendere di più).

In questo senso, il sistema informativo rappresenta la \textbf{visione complessiva delle informazioni} necessarie al funzionamento dell’organizzazione.


\begin{center}
\resizebox{0.95\textwidth}{!}{
\begin{tikzpicture}[node distance=2cm, every node/.style={draw, rounded corners, align=center, minimum width=3cm, minimum height=1cm}]
    % Nodo centrale
    \node[fill=yellow!30] (Azienda) {Azienda di Scarpe\\Sistema Informativo};
    
    % Nodi sopra
    \node [above left=of Azienda, fill=green!20] (Fornitori) {Fornitori\\(chi, prezzi, consegne)};
    \node [above right=of Azienda, fill=green!20] (Marketing) {Marketing\\(promozioni, pubblicità)};
    
    % Nodi sotto
    \node [below left=of Azienda, fill=green!20] (Clienti) {Clienti\\(dati anagrafici, acquisti, preferenze)};
    \node [below=of Azienda, fill=green!20] (Dipendenti) {Dipendenti\\(orari, ruoli, stipendi)};
    \node [below right=of Azienda, fill=green!20] (Prodotti) {Prodotti\\(modelli, materiali, quantità)};
    \node [left=of Clienti, xshift=-2.5cm, fill=green!20] (Acquisti) {Acquisti e Vendite\\(quantità vendute, riordini)};
    
    % Frecce dal nodo centrale
    \draw[->, thick] (Azienda) -- (Fornitori);
    \draw[->, thick] (Azienda) -- (Marketing);
    \draw[->, thick] (Azienda) -- (Clienti);
    \draw[->, thick] (Azienda) -- (Dipendenti);
    \draw[->, thick] (Azienda) -- (Prodotti);
    \draw[->, thick] (Azienda) -- (Acquisti);
\end{tikzpicture}
}
\end{center}

Un \textbf{Sistema Informatico} è invece la \textbf{parte tecnologica} del sistema informativo: l’insieme di computer, software e reti che permettono di \textbf{automatizzare i processi} e di \textbf{gestire i dati in modo più efficiente, sicuro e affidabile}.

La digitalizzazione consente di:
\begin{itemize}
    \item automatizzare operazioni ripetitive;
    \item ridurre errori e tempi di elaborazione;
    \item gestire grandi quantità di dati;
    \item rendere le informazioni facilmente accessibili e aggiornabili.
\end{itemize}

È proprio in questo contesto che entrano in gioco le \textbf{basi di dati}: esse rappresentano lo strumento principale attraverso cui il sistema informatico realizza una gestione strutturata, coerente e persistente dei dati all’interno del sistema informativo.



%==============================================================================
\subsection{L'evoluzione della gestione dei dati}
%==============================================================================

Fino a migliaia di anni fa, le informazioni venivano registrate esclusivamente su supporti fisici. Già nell’antica Roma, ad esempio, si utilizzavano tavole, pergamene e registri scritti per tenere traccia di censimenti, tributi, proprietà e atti amministrativi. Per secoli, e fino all’arrivo dei computer, ogni organizzazione ha gestito il proprio sistema informativo in forma cartacea, attraverso archivi fisici conservati negli uffici.
L’introduzione della tecnologia informatica ha permesso di digitalizzare queste informazioni, rendendo possibile la loro memorizzazione, gestione e utilizzo tramite sistemi informatici, con enormi vantaggi in termini di velocità, affidabilità e accessibilità dei dati.

\subsubsection{L’approccio basato sui File (File System)}
Nei primi sistemi informatici, i dati erano gestiti separatamente da ciascun programma, mediante file dedicati, sia di tipo testuale sia binario. Ad esempio, il software per la gestione degli stipendi conservava i dati dei dipendenti in un file specifico, mentre il programma destinato alla gestione dei corsi di formazione utilizzava un file differente.

Questo approccio risulta semplice e veloce da implementare, ma presenta notevoli limitazioni in termini di gestione e manutenzione dei dati. In contesti di maggiore complessità, come nel caso di un’organizzazione di grandi dimensioni, emergono problematiche significative. Ad esempio, risulta complicato coordinare le informazioni sulle vendite di un prodotto quando i dati dei clienti sono distribuiti su file distinti e, talvolta, su sedi differenti.

\begin{figure}[!h]
    \centering
    \includegraphics[width=0.3\textwidth]{images/independent-files.png}
    \caption{Ogni applicazione gestisce i propri file in modo indipendente}
\end{figure}

Inoltre, nei sistemi completamente informatizzati odierni, come quelli utilizzati per le transazioni bancarie o per la gestione di piattaforme e-commerce, è necessario garantire requisiti fondamentali quali sicurezza, affidabilità e accesso concorrente da parte di più utenti. Ogni applicazione implementa la sua logica per gestire questi aspetti, il che può portare a inefficienze e a un aumento della complessità del sistema.


\subsubsection{L'approccio DBMS (Database Management System)}
Per gestire tutti i problemi che emergono con i sistemi basati su file, oggi ci si affida a software specializzati chiamati DBMS (Database Management System).
\begin{figure}[!h]
    \centering
    \includegraphics[width=0.45\textwidth]{images/integrated-database-DBMS.png}
    \caption{App che dialogano con un DBMS}
\end{figure}
Questi programmi permettono di organizzare e gestire i dati in maniera professionale, garantendo che tutto sia corretto, aggiornato e accessibile in sicurezza anche a più utenti contemporaneamente. In pratica, il DBMS fa da “gestore centrale” dei dati, evitando confusione, duplicazioni e errori. Ogni applicazione che ha bisogno di dati (come un sito web, un’app bancaria o un sistema di gestione scolastica) si collega al DBMS per leggere o scrivere le informazioni necessarie.

\newpage
%==============================================================================
%==============================================================================
%==============================================================================
\section{Database Management System (DBMS)}
%==============================================================================

Un Database Management System (DBMS) è un software progettato per gestire collezioni di dati in modo affidabile, efficiente e sicuro.  
Per essere utilizzabile in contesti reali (aziendali, scolastici o industriali), un DBMS deve garantire alcune caratteristiche fondamentali, che risolvono i principali problemi della gestione dei dati su larga scala.

\begin{center}
\begin{tikzpicture}[
    node distance=2cm,
    every node/.style={
        draw,
        rounded corners,
        align=center,
        minimum width=3cm,
        minimum height=0.9cm
    }
]

% Nodo centrale
\node[fill=yellow!30] (DBMS) {\textbf{DBMS}\\Database Management System};

% Nodi esterni
\node[above=of DBMS, fill=green!20] (Eff) {Efficienza\\(velocità e grandi volumi di dati)};
\node[right=of DBMS, fill=green!20] (Conc) {Concorrenza\\(più utenti)};
\node[below=of DBMS, fill=green!20] (Aff) {Affidabilità\\(tutto o niente)};
\node[left=of DBMS, fill=green!20] (Sec) {Sicurezza\\(accessi)};

% Frecce
\draw[->, thick] (DBMS) -- (Eff);
\draw[->, thick] (DBMS) -- (Conc);
\draw[->, thick] (DBMS) -- (Aff);
\draw[->, thick] (DBMS) -- (Sec);

\end{tikzpicture}
\end{center}

\subsection{Caratteristiche di un DBMS}
\subsection*{Efficienza}
Un DBMS deve essere in grado di gestire grandi quantità di dati mantenendo tempi di risposta molto ridotti.  
I dati devono essere persistenti, cioè conservati nel tempo anche dopo la chiusura dei programmi o lo spegnimento del sistema, e devono poter essere recuperati rapidamente quando richiesti.

Nella realtà moderna, i volumi di dati possono raggiungere dimensioni enormi. Alcuni esempi indicativi sono:
\begin{itemize}
    \item Amazon Prime: oltre 240 milioni di abbonati;
    \item Facebook: più di 3 miliardi di utenti attivi mensili;
    \item Instagram: oltre 1,3 miliardi di immagini caricate ogni giorno.
\end{itemize}

Anche in presenza di questi volumi, un DBMS deve consentire operazioni rapide, come la ricerca di un contenuto o l’accesso a un profilo utente, senza che l’utente percepisca rallentamenti.

\subsection*{Concorrenza}
Un DBMS deve gestire correttamente l’accesso simultaneo ai dati da parte di più utenti o applicazioni. Se più operazioni vengono eseguite sugli stessi dati nello stesso istante, il rischio è ottenere risultati errati o incoerenti.  

\subsubsection*{Esempio di concorrenza: prelievi simultanei}

Si consideri un sistema informativo bancario in cui due utenti effettuano un prelievo dallo stesso conto corrente nello stesso istante.

\begin{itemize}
    \item Saldo iniziale del conto X: 120 €
    \item Operazione OP1: prelievo di 100 €
    \item Operazione OP2: prelievo di 80 €
\end{itemize}


\begin{center}
\begin{tabular}{c c c}
\toprule
Tempo & Operazione & Valore di X \\
\midrule
$t_1$ & OP1: Leggi $saldo$ & 120 \\
$t_2$ & OP2: Leggi $saldo$ & 120 \\
$t_3$ & OP1: Calcola $saldo - 100$ & 120 \\
$t_4$ & OP2: Calcola $saldo - 80$ & 120 \\
$t_5$ & OP1: Scrivi $saldo finale$ & 20 \\
$t_6$ & OP2: Scrivi $saldo finale$ & 40 \\
\bottomrule
\end{tabular}
\end{center}

\begin{center}
$Saldo finale = 40 \rightarrow ERRORE!$
\end{center}


Il sistema dovrebbe permettere che solamente una delle due operazioni venga completata, poiché il saldo non è sufficiente per entrambe.

Per risolvere questo problema, i DBMS utilizzano meccanismi di controllo della concorrenza che garantiscono che l’esecuzione simultanea delle operazioni produca lo stesso risultato di un’esecuzione una alla volta (seriale).

\subsection*{Affidabilità}
Un DBMS deve garantire che i dati rimangano corretti e coerenti anche in presenza di errori, interruzioni di corrente o arresti improvvisi del sistema.  
Molte operazioni sui dati sono infatti critiche e non possono essere eseguite parzialmente: devono rispettare la regola del \textbf{tutto o niente}.


\subsubsection*{Esempio di concorrenza: prelievi simultanei}
Si consideri un sistema informativo bancario che gestisce il trasferimento di denaro tra due conti correnti. Un trasferimento non è una singola operazione, ma una sequenza di passi che devono essere eseguiti in modo coordinato.

\bigskip
\begin{itemize}
    \item Conto A (mittente): saldo iniziale 500 €
    \item Conto B (destinatario): saldo iniziale 300 €
    \item Importo del trasferimento: 200 €
\end{itemize}
\bigskip
Il trasferimento prevede due operazioni fondamentali:
\begin{itemize}
    \item addebitare 200 € dal conto A;
    \item accreditare 200 € sul conto B.
\end{itemize}
\bigskip
Se il sistema dovesse interrompersi dopo l’addebito ma prima dell’accredito, il database si troverebbe in uno stato non corretto: il denaro risulterebbe sottratto dal conto A ma non accreditato sul conto B.\\\\
Un DBMS affidabile deve garantire che \textbf{solo uno dei due scenari seguenti}:
\begin{itemize}
    \item entrambe le operazioni vengano completate correttamente;
    \item nessuna delle due operazioni venga effettuata.
\end{itemize}

Per ottenere questo comportamento, il DBMS utilizza meccanismi di gestione delle \textbf{transazioni}, che permettono di annullare automaticamente le operazioni incomplete e ripristinare uno stato consistente dei dati.

\subsection*{Sicurezza}
Un DBMS deve proteggere i dati da accessi non autorizzati.  
Non tutti gli utenti devono poter leggere o modificare tutte le informazioni presenti nel database.

Per questo motivo, i DBMS implementano sistemi di autorizzazione basati su permessi, che stabiliscono:
\begin{itemize}
    \item quali dati un utente può visualizzare;
    \item quali operazioni può eseguire (lettura, inserimento, modifica, cancellazione).
\end{itemize}


%\subsubsection*{Proprietà ACID (da non mettere qui)}
%Per garantire l’integrità dei dati, un DBMS si basa sul concetto di transazione: una sequenza %di operazioni che deve essere completata interamente oppure annullata completamente in caso di %errore. Le proprietà fondamentali delle transazioni sono note con l’acronimo \textbf{ACID}:
%
%\begin{itemize}
%    \item \textbf{Atomicità:} ogni transazione è indivisibile; o viene completata %completamente, o non ha alcun effetto.
%    \item \textbf{Coerenza:} le transazioni portano il database da uno stato valido a un altro %stato valido.
%    \item \textbf{Isolamento:} le operazioni eseguite da un utente non interferiscono con %quelle di altri utenti fino al completamento della transazione.
%    \item \textbf{Durabilità:} una volta confermate, le modifiche ai dati rimangono permanenti %anche in caso di guasti del sistema.
%\end{itemize}
%
%\begin{center}
%\begin{tikzpicture}[node distance=2cm, every node/.style={draw, rounded corners, align=center, %minimum width=3cm, minimum height=1cm}]
%    % Nodo centrale
%    \node[fill=yellow!30] (ACID) {Transazione ACID};
%
%    % Ramificazioni
%    \node [above left=of ACID, fill=green!20] (A) {Atomicità};
%    \node [above right=of ACID, fill=green!20] (C) {Coerenza};
%    \node [below left=of ACID, fill=green!20] (I) {Isolamento};
%    \node [below right=of ACID, fill=green!20] (D) {Durabilità};
%
%    % Frecce
%    \draw[->, thick] (ACID) -- (A);
%    \draw[->, thick] (ACID) -- (C);
%    \draw[->, thick] (ACID) -- (I);
%    \draw[->, thick] (ACID) -- (D);
%\end{tikzpicture}
%\end{center}

\newpage
%==============================================================================
\section{L'astrazione fornita dai DBMS}
%==============================================================================


I Database Management System (DBMS) sono progettati secondo una \textbf{struttura a livelli di astrazione}, che consente di separare nettamente le problematiche legate alla memorizzazione fisica dei dati da quelle relative alla loro organizzazione logica e alla loro presentazione agli utenti o alle applicazioni.

Questa architettura a livelli permette a chi utilizza il database (sia persone sia programmi) di \textbf{interagire con i dati senza doversi preoccupare di come essi sono effettivamente salvati nei dispositivi di memoria}. In altre parole, gli utenti possono concentrarsi su \textit{cosa} vogliono ottenere dal database, senza dover sapere \textit{come} i dati sono archiviati o gestiti internamente. Ad esempio possono chiedere \textit{"dammi i dati di tutti i clienti"} oppure \textit{"dammi i prodotti più venduti questo mese"}, e non dire \textit{recupera dati dal file x}.


\begin{figure}[!h]
    \centering
    \includegraphics[width=1\textwidth]{images/language dbms.png}
    \caption{Ogni utente/applicazione interagisce con il DBMS tramite un linguaggio specifico che nasconde dettagli}
\end{figure}


\subsection{Livelli di astrazione di un DBMS}
\begin{enumerate}
    \item \textbf{Livello Fisico:} È il livello più basso. Riguarda come i dati sono effettivamente memorizzati nei file su disco. A questo livello si gestiscono dettagli tecnici come la compressione, la disposizione dei record nei file, le strategie di accesso rapido. Gli utenti e gli sviluppatori di applicazioni in genere non si occupano di questo livello, che è gestito direttamente dal DBMS.
    \item \textbf{Livello Logico:} Descrive la struttura logica globale del database e non dipende dai dettagli fisici di memorizzazione. Si parla di tabelle, attributi, relazioni e vincoli. Gli sviluppatori e i progettisti del database lavorano principalmente a questo livello, definendo schemi, relazioni e regole che modellano la realtà da rappresentare.
    \item \textbf{Livello Esterno (o delle Viste):} È il livello più vicino agli \textbf{utenti e alle applicazioni}. Qui si definiscono le ``\textbf{viste}'' personalizzate, cioè porzioni del database che ciascun utente può vedere e manipolare. Ogni utente/applicazione può avere una vista diversa, limitata ai dati che gli servono o che è autorizzato a consultare. Questo livello è fondamentale per la sicurezza e la semplicità d’uso.
\end{enumerate}

\begin{figure}[!h]
    \centering
    \includegraphics[width=0.4\textwidth]{images/three-levels-database-architecture.jpg}
    \caption{La struttura a tre livelli di un DBMS}
\end{figure}



\subsection*{Livello Logico di un DBMS}
Il livello logico descrive \textbf{come i dati sono organizzati e strutturati, indipendentemente da come vengono memorizzati fisicamente}. 
Questo livello è fondamentale perché definisce la \textbf{struttura globale} del database, ovvero lo \textbf{schema logico}, che rappresenta la realtà che si vuole modellare (ad esempio: utenti, prodotti, ordini, ecc.).

In pratica, il livello logico permette di modellare la realtà in modo astratto e indipendente dai dettagli tecnici di memorizzazione. Questo favorisce la \textbf{modularità} e la \textbf{manutenibilità} del database: è possibile modificare la struttura fisica dei dati senza cambiare la loro organizzazione logica, e viceversa.

Il modello più comune è il  \textbf{modello relazionale}, che utilizza \textbf{relazioni} (tabelle) per rappresentare le informazioni.

\subsubsection*{Esempio}  
Supponiamo di avere un social network. Nel modello relazionale, potremmo avere la seguente tabella per gestire e memorizzare gli utenti:

\begin{center}
\begin{tabular}{|c|c|c|c|}
\hline
\textbf{Codice Utente} & \textbf{Nome} & \textbf{Cognome} & \textbf{Email} \\
\hline
1 & Marco & Rossi & marco.rossi@mail.com \\
2 & Anna & Bianchi & anna.bianchi@mail.com \\
3 & Luca & Verdi & luca.verdi@mail.com \\
\hline
\end{tabular}

\end{center}
 
\subsection{Il linguaggio dei DBMS}
Per interagire con un database, il linguaggio più diffuso è \textbf{SQL} (Structured Query Language), che consente di comunicare con il DBMS in maniera semplice e orientata ai dati.

Il linguaggio SQL è utilizzato da tutti i database relazionali, anche se con alcune variazioni specifiche per ciascun DBMS (ad esempio MySQL, PostgreSQL, Oracle, SQL Server). Basta quindi imparare un solo linguaggio per poter lavorare con diversi sistemi di basi di dati.


SQL si suddivide in tre principali categorie di comandi:

\begin{itemize}
    \item \textbf{DDL (Data Definition Language):} comandi per definire e modificare la struttura del database, ad esempio \texttt{CREATE TABLE}, \texttt{ALTER TABLE}, \texttt{DROP TABLE}.
    \item \textbf{DML (Data Manipulation Language):} comandi per inserire, aggiornare e cancellare i dati nelle tabelle, ad esempio \texttt{INSERT}, \texttt{UPDATE}, \texttt{DELETE}.
    \item \textbf{DQL (Data Query Language):} comandi per interrogare il database e ottenere informazioni, principalmente tramite \texttt{SELECT}.
\end{itemize}

Grazie a SQL, è possibile lavorare direttamente sui dati in modo astratto e uniforme, senza dover conoscere i dettagli fisici di memorizzazione, come file o indici su disco.



\newpage
\newpage
\section{Il modello relazionale}
%==============================================================================

Il modello relazionale è stato introdotto negli anni '70 dal professor \textbf{Edgar F. Codd} presso IBM.  
La sua idea rivoluzionaria era di rappresentare le informazioni tramite tabelle (relazioni) anziché attraverso strutture ad hoc o file sequenziali.  
I database che seguono questo modello sono chiamati \textbf{RDBMS} (Relational Database Management System) e costituiscono ancora oggi lo standard per la maggior parte delle applicazioni professionali.

L'idea principale del modello relazionale è semplice: rappresentare le informazioni tramite \textbf{tabelle}, chiamate \textbf{relazioni}, dove ogni riga corrisponde a un oggetto reale e ogni colonna a una proprietà di quell'oggetto.


Per rendere l'idea, consideriamo un contesto scolastico. Gli oggetti principali sono studenti, materie e voti.  

\begin{itemize}
    \item \textbf{Studente:} identificato da matricola, nome e cognome.
    \item \textbf{Materia:} identificata da codice e nome.
    \item \textbf{Voto:} associa uno studente a una materia in un certo periodo, con il punteggio ottenuto.
\end{itemize}

Rappresentiamo queste entità come tabelle:

\begin{center}
\begin{tabular}{|c|c|c|}
\hline
\textbf{Matricola} & \textbf{Nome} & \textbf{Cognome} \\
\hline
101 & Marco & Rossi \\
102 & Anna & Bianchi \\
103 & Luca & Verdi \\
\hline
\end{tabular}
\hspace{1cm}
\begin{tabular}{|c|c|}
\hline
\textbf{Codice} & \textbf{Materia} \\
\hline
MAT01 & Matematica \\
INF02 & Informatica \\
FIS03 & Fisica \\
\hline
\end{tabular}
\end{center}

\begin{center}
\begin{tabular}{|c|c|c|}
\hline
\textbf{Matricola} & \textbf{Codice Materia} & \textbf{Voto} \\
\hline
101 & MAT01 & 8 \\
102 & INF02 & 7 \\
103 & FIS03 & 9 \\
\hline
\end{tabular}
\end{center}

In termini formali del modello relazionale, la tabella è chiamata \textbf{relazione}.  
Le colonne della tabella sono chiamate \textbf{attributi}, mentre le righe corrispondono alle \textbf{tuple} o \textbf{istanze della relazione}.  
La relazione ha uno \textbf{schema}, che definisce i nomi degli attributi e i tipi di dati associati. Lo schema resta fisso, mentre le istanze della relazione possono cambiare nel tempo.

Queste tabelle rappresentano la \textbf{struttura logica} dei dati: ogni riga è una \textbf{tupla}, ogni colonna un \textbf{attributo}, e insieme formano una relazione.

\subsubsection*{Schema e istanza}

Lo \textbf{schema} della relazione definisce i nomi delle colonne e i tipi di dati, ad esempio:
\begin{center}
\code{Studente(Matricola, Nome, Cognome)}
\end{center}
L'\textbf{istanza} della relazione è l'insieme delle righe effettivamente presenti in un certo momento:
\begin{center}
\code{(101, Marco, Rossi), (102, Anna, Bianchi), (103, Luca, Verdi)}
\end{center}


Questa distinzione tra schema e istanza è fondamentale: lo schema descrive la struttura, mentre l'istanza è ciò che contiene i dati reali.

\subsubsection*{Vincoli sull'ordine dei dati}
È importante sottolineare che nel modello relazionale non esistono vincoli sull'ordine dei dati: 
l'ordinamento delle righe è irrilevante, così come l'ordinamento delle colonne.  
Una relazione è definita dal contenuto delle tuple e dagli attributi, non dalla loro posizione nella tabella.


\subsubsection*{Vincoli sulla relazione}
na relazione ha alcune regole fondamentali:

\begin{itemize}
    \item \textbf{Ogni attributo deve essere unico:} non possono esistere due colonne con lo stesso nome all'interno della stessa relazione.
    \item \textbf{Ogni tupla deve essere unica:} non possono esistere due righe identiche all'interno della relazione.
    \item \textbf{Omogeneità dei dati per colonna:} tutti i valori di una colonna devono appartenere allo stesso dominio (tipo di dato).  
    \textbf{Esempio:} nella colonna \texttt{Matricola} devono esserci solo numeri interi, mentre nella colonna \texttt{Nome} solo stringhe di testo.
\end{itemize}

\subsection{Formalizzazione del Modello Relazionale}
Il termine \textbf{relazione} non è casuale: deriva dalla \textit{teoria degli insiemi} in matematica.  
In matematica, una relazione tra insiemi è un insieme di n-uple ordinate, che collegano elementi di insiemi diversi.  
Allo stesso modo, una tabella in un database relazionale rappresenta un insieme di tuple che collegano valori di attributi, ciascuno dei quali appartiene a un dominio specifico.

\subsubsection{Definizione di Relazione}

Sia dato un insieme finito di attributi \( \{A_1, A_2, \dots, A_n\} \), 
dove ciascun attributo \(A_i\) è associato a un dominio \(D_i\) di valori possibili.


\vspace{0.5cm}

Una \textbf{relazione} \(R\) su questi attributi è un insieme di tuple \(t\) tali che
\[
t = (v_1, v_2, \dots, v_n), \quad \text{con } v_i \in D_i \text{ per } i=1,\dots,n.
\]

\vspace{0.2cm}

In altre parole, una relazione è un sottoinsieme del prodotto cartesiano dei domini:
\[
R \subseteq D_1 \times D_2 \times \dots \times D_n
\]
\textbf{Nota:}  
\begin{itemize}

    \item Ogni tupla \(t \in R\) corrisponde a una riga della relazione.  

    \item Ogni attributo \(A_i\) corrisponde a una colonna.  

    \item Non ci sono tuple duplicate: \(R\) è un insieme.  

    \item Lo schema della relazione è dato dall'insieme \(\{A_1, A_2, \dots, A_n\}\) e dai domini associati \(\{D_1, D_2, \dots, D_n\}\).
\end{itemize}
\textbf{Esempio:} consideriamo due insiemi
\[
A = \{101, 102, 103\}, \quad B = \{\text{Matematica}, \text{Informatica}, \text{Fisica}\}
\]
e una relazione \texttt{Voto} tra studenti e materie:
\[
\text{Voto} = \{ (101, \text{Matematica}, 8), (102, \text{Informatica}, 7), (103, \text{Fisica}, 9) \}
\]
Ogni tupla associa uno studente a una materia e a un voto, proprio come una n-upla matematica.


%==============================================================================
%==============================================================================
%==============================================================================

\subsection{Vincoli d'integrità intra-relazionali}
%==============================================================================

Nel modello relazionale, oltre a definire gli attributi e le tuple, è fondamentale imporre regole che garantiscano **l'unicità e la coerenza dei dati**. Queste regole si chiamano \textbf{vincoli d'integrità}.

% Vincoli di tupla
\subsubsection{Vincoli di tupla}

I \textbf{vincoli di tupla} (o vincoli di ennupla) impongono condizioni che devono essere soddisfatte \textbf{da ogni singola tupla} della relazione, indipendentemente dalle altre.

Questi vincoli riguardano:
\begin{itemize}
    \item il \textbf{dominio} dei valori ammessi;
    \item la \textbf{presenza obbligatoria} di un valore (non nullo);
    \item eventuali \textbf{intervalli o condizioni logiche}.
\end{itemize}

\textbf{Esempio 1 -- Vincolo di dominio:}

Relazione \texttt{Voto(Matricola, CodiceMateria, Voto)}:
\begin{center}
\begin{tabular}{|c|c|c|}
\hline
\textbf{Matricola} & \textbf{CodiceMateria} & \textbf{Voto} \\
\hline
101 & MAT01 & 8 \\
102 & INF02 & 11 \\
103 & FIS03 & 9 \\
\hline
\end{tabular}
\end{center}

Il valore \texttt{11} viola il vincolo di tupla, perché il voto deve appartenere all’intervallo \([0,10]\).

\bigskip

\textbf{Esempio 2 -- Vincolo di non nullità:}

Relazione \texttt{Studente(Matricola, Nome, Cognome)}:
\begin{center}
\begin{tabular}{|c|c|c|}
\hline
\textbf{Matricola} & \textbf{Nome} & \textbf{Cognome} \\
\hline
101 & Marco & Rossi \\
102 &  & Bianchi \\
103 & Luca & Verdi \\
\hline
\end{tabular}
\end{center}

La seconda tupla viola il vincolo di tupla perché l’attributo \texttt{Nome} non può essere vuoto o nullo.

\bigskip

In generale, un vincolo di tupla deve essere verificato \textbf{per ogni riga singolarmente}, senza confrontarla con le altre tuple della relazione.
\subsubsection{Vincoli di chiave (Primary Key)}

Nel modello relazionale, il concetto di \textbf{chiave} nasce dall’esigenza di identificare in modo univoco le ennuple (tuple) di una relazione. Infatti per \textbf{chiave di una relazione} si intende un insieme di attributi che consente di identificare in maniera univoca le ennuple di una relazione.\mbox{}\\\\
Per essere più precisi, definiamo due concetti fondamentali:
\begin{itemize}
    
    \item \textbf{Superchiave}: una superchiave è un insieme di attributi che identifica in modo univoco le ennuple di una relazione, ma che può contenere attributi ridondanti, cioè non strettamente necessari all’identificazione.
    
    \item \textbf{Superchiave minimale}: una superchiave minimale è una superchiave tale che non esiste un’altra superchiave \(K'\) contenuta in essa.
    In altre parole, è una superchiave priva di attributi superflui.
\end{itemize}

\textbf{Esempio:} consideriamo la relazione \texttt{Studente(Matricola, Nome, Cognome)}.


% Esempio tabulare delle tuple della relazione Studente 
\begin{center}
\begin{tabular}{|c|c|c|}
\hline
\textbf{Matricola} & \textbf{Nome} & \textbf{Cognome} \\
\hline
101 & Marco & Rossi \\
102 & Anna & Bianchi \\
103 & Luca & Verdi \\
104 & Anna & Verdi \\
\hline
\end{tabular}
\end{center}
\begin{itemize}
    \item \{\texttt{Matricola}\} è una superchiave minimale, quindi una chiave.
    \item \{\texttt{Matricola, Nome}\} è una superchiave, ma non minimale.
    \item \{\texttt{Matricola, Nome, Cognome}\} è ancora una superchiave, ma contiene attributi ridondanti.
\end{itemize}
Una relazione può avere più chiavi (superchiavi minimali). Tra queste, una viene scelta come \textbf{chiave primaria}.



\subsubsection*{Esiste sempre almeno una superchiave per ogni relazione?}

\textbf{Sì.}

\begin{center}
\begin{tabular}{|c|c|c|}
\hline
\textbf{Matricola} & \textbf{Nome} & \textbf{Cognome} \\
\hline
101 & Marco & Rossi \\
102 & Anna & Bianchi \\
103 & Luca & Verdi \\
\hline
\end{tabular}
\end{center}
L’insieme di tutti gli attributi della relazione, in questo caso \{\texttt{Matricola, Nome, Cognome}\} è sempre una superchiave.

\bigskip

\subsubsection*{Possono esistere più superchiavi per la stessa relazione?}

\textbf{Sì.}

\begin{center}
\begin{tabular}{|c|c|c|}
\hline
\textbf{Matricola} & \textbf{Nome} & \textbf{Cognome} \\
\hline
101 & Marco & Rossi \\
102 & Anna & Bianchi \\
103 & Luca & Verdi \\
\hline
\end{tabular}
\end{center}
Sono superchiavi:
\begin{itemize}
    \item \{\texttt{Matricola}\}
    \item \{\texttt{Matricola, Nome}\}
    \item \{\texttt{Matricola, Cognome}\}
    \item \{\texttt{Matricola, Nome, Cognome}\}
\end{itemize}

\bigskip

\subsubsection*{Esiste sempre almeno una chiave per ogni relazione?}

\textbf{Sì.}

\begin{center}
\begin{tabular}{|c|c|c|}
\hline
\textbf{Matricola} & \textbf{Nome} & \textbf{Cognome} \\
\hline
101 & Marco & Rossi \\
102 & Anna & Bianchi \\
103 & Luca & Verdi \\
\hline
\end{tabular}
\end{center}

L’attributo \texttt{Matricola} identifica univocamente ogni tupla ed è una superchiave minimale, quindi una chiave.

\bigskip

\subsubsection*{Possono esistere più chiavi per la stessa relazione?}

\textbf{Sì.}

\begin{center}
\begin{tabular}{|c|c|c|}
\hline
\textbf{CodiceUtente} & \textbf{Email} & \textbf{Username} \\
\hline
U001 & marco@mail.com & marcoR \\
U002 & anna@mail.com & annaB \\
U003 & luca@mail.com & lucaV \\
\hline
\end{tabular}
\end{center}

In questa relazione sono chiavi:
\begin{itemize}
    \item \{\texttt{CodiceUtente}\}
    \item \{\texttt{Email}\}
    \item \{\texttt{Username}\}
\end{itemize}
Una di queste viene scelta come chiave primaria.


\newpage
\subsection{Vincoli d'integrità inter-relazionali}

\subsubsection{Chiavi esterne (Foreign Key)}

Una chiave esterna è un vincolo che garantisce che i valori inseriti in una relazione corrispondano a valori presenti in un’altra relazione, mantenendo così l’integrità referenziale del database.

\paragraph{Passo 1 -- Relazioni separate}\mbox{}\\
Nel modello relazionale, una base di dati è composta da \textbf{più relazioni} collegate tra loro.  

\vspace{0.5cm}
\textbf{Utente}
\begin{center}
\begin{tabular}{|c|c|c|}
\hline
\textbf{IDUtente} & \textbf{Nome} & \textbf{Cognome} \\
\hline
1 & Marco & Rossi \\
2 & Anna & Bianchi \\
3 & Luca & Verdi \\
\hline
\end{tabular}
\end{center}

\textbf{Prodotto}
\begin{center}
\begin{tabular}{|c|c|c|}
\hline
\textbf{IDProdotto} & \textbf{NomeProdotto} & \textbf{Prezzo} \\
\hline
101 & Smartphone & 599.00 \\
102 & Laptop & 1200.00 \\
103 & Tablet & 399.00 \\
\hline
\end{tabular}
\end{center}

In entrambe le relazioni, \texttt{IDUtente} e \texttt{IDProdotto} sono chiavi primarie.

\paragraph{Passo 2 -- Relazione che collega le altre}\mbox{}\\
Introduciamo ora una terza relazione che registra gli acquisti degli utenti:

\vspace{0.5cm}
\textbf{Acquisto}
\begin{center}
\begin{tabular}{|c|c|c|}
\hline
\textbf{IDUtente} & \textbf{IDProdotto} & \textbf{Quantità} \\
\hline
1 & 101 & 1 \\
2 & 102 & 2 \\
3 & 103 & 1 \\
1 & 102 & 1 \\
\hline
\end{tabular}
\end{center}

\paragraph{Passo 3 -- Identificazione delle chiavi esterne}\mbox{}\\
Nella relazione \texttt{Acquisto}:
\begin{itemize}
    \item \texttt{IDUtente} è una \textbf{chiave esterna} che fa riferimento alla chiave primaria \texttt{IDUtente} della relazione \texttt{Utente};
    \item \texttt{IDProdotto} è una \textbf{chiave esterna} che fa riferimento alla chiave primaria \texttt{IDProdotto} della relazione \texttt{Prodotto}.
\end{itemize}

\bigskip
\paragraph{Perché servono le chiavi esterne}\mbox{}\\
Le chiavi esterne garantiscono l’\textbf{integrità referenziale}, cioè:
\begin{itemize}
    \item non è possibile registrare un acquisto per un utente che non esiste;
    \item non è possibile registrare un acquisto per un prodotto che non esiste.
\end{itemize}
Ogni valore di chiave esterna deve quindi corrispondere a un valore presente nella relazione di riferimento.















%====================================================================
%====================================================================
%====================================================================
\newpage
%==============================================================================
\section{Il linguaggio dei DBMS: SQL}
%==============================================================================

\subsection{Introduzione a SQL: sintassi e principi di base}

Per interagire con una base di dati relazionale si utilizza il linguaggio \textbf{SQL} (\textit{Structured Query Language}).  
SQL è un linguaggio dichiarativo, cioè permette di \textbf{dire cosa vogliamo ottenere} dal database, senza preoccuparci di come il DBMS eseguirà le operazioni.  
Tutti i principali sistemi di gestione di basi di dati (come MySQL, PostgreSQL, Oracle, SQL Server) utilizzano SQL come linguaggio standard (anche se possono esserci piccole differenze tra i vari sistemi).

\medskip
\textbf{Cosa è SQL?}
\begin{itemize}
    \item SQL è un linguaggio pensato per \textbf{creare}, \textbf{modificare}, \textbf{gestire} e \textbf{interrogare} le basi di dati relazionali.
    \item Permette di esprimere in modo semplice e diretto le operazioni che vogliamo eseguire sui dati (ad esempio: cercare, inserire, aggiornare, cancellare, creare tabelle, ecc.).
\end{itemize}

\textbf{Principi fondamentali della sintassi SQL:}
\begin{itemize}
    \item \textbf{Ogni istruzione termina con il punto e virgola \code{;}}\\
    In SQL, ogni comando (o "query") deve essere terminato dal simbolo \code{;} per indicare al DBMS la fine dell'istruzione.
    \begin{verbatim}
    SELECT * FROM Studente;
    \end{verbatim}
    \item \textbf{Non fa distinzione tra maiuscole e minuscole}\\
    SQL è \textbf{case-insensitive}: le parole chiave e i nomi delle tabelle/colonne possono essere scritti sia in maiuscolo che in minuscolo, indifferentemente.
    \begin{verbatim}
    select * from studente;
    SELECT * FROM Studente;
    \end{verbatim}
    Le due istruzioni sopra sono equivalenti. Per convenzione, spesso si scrivono le parole chiave in maiuscolo (\code{SELECT, FROM, WHERE}) e i nomi delle tabelle/colonne in minuscolo o con la maiuscola iniziale.
    \item \textbf{Commenti in SQL}\\
    È possibile inserire commenti per spiegare il codice:
    \begin{itemize}
        \item Commento su una riga: \code{-- questo è un commento}
        \item Commento su più righe: \code{/* questo è un commento su più righe */}
    \end{itemize}
    \begin{verbatim}
    -- Seleziona tutti gli studenti
    SELECT * FROM Studente;
    \end{verbatim}
    \item \textbf{Struttura di una istruzione SQL}\\
    Una tipica istruzione SQL segue la struttura:
    \begin{verbatim}
    PAROLA_CHIAVE parametri ... ;
    \end{verbatim}
    Ad esempio:
    \begin{verbatim}
    SELECT Nome, Cognome FROM Studente WHERE Matricola = 102;
    \end{verbatim}
    \item \textbf{Spazi e a capo}\\
    Gli spazi e i ritorni a capo sono ignorati da SQL (servono solo per rendere il codice più leggibile).
\end{itemize}

\textbf{Categorie principali dei comandi SQL:}
\begin{itemize}
    \item \textbf{DQL (Data Query Language):} per \textbf{interrogare} e leggere i dati (es: \code{SELECT}).
    \item \textbf{DML (Data Manipulation Language):} per \textbf{inserire, modificare e cancellare} i dati (es: \code{INSERT}, \code{UPDATE}, \code{DELETE}).
    \item \textbf{DDL (Data Definition Language):} per \textbf{definire e modificare la struttura} delle tabelle e del database (es: \code{CREATE TABLE}, \code{ALTER TABLE}, \code{DROP TABLE}).
\end{itemize}

\textbf{In sintesi:}  
SQL è un linguaggio semplice e potente, con una sintassi regolare e facilmente leggibile. È importante abituarsi a scrivere istruzioni chiare, terminare sempre con il punto e virgola, e usare i commenti per rendere il codice comprensibile.

\medskip
Di seguito vediamo ciascuna categoria con spiegazioni semplici ed esempi pratici.







%==============================================================================
\newpage
\section{SQL DDL (Data Definition Language)}
%==============================================================================

\textbf{DDL} (Data Definition Language) è la parte di SQL che serve per \textbf{definire e modificare la struttura del database}, come creare, eliminare o modificare tabelle e altri oggetti.  
I principali comandi DDL sono:

\begin{itemize}
    \item \code{CREATE} -- creare tabelle o altri oggetti;
    \item \code{DROP} -- eliminare tabelle o altri oggetti;
    \item \code{ALTER} -- modificare la struttura di una tabella esistente.
\end{itemize}

%==============================================================================
\subsection{Creare tabelle e altri oggetti con CREATE}
%==============================================================================

Il comando \code{CREATE} serve per creare nuovi oggetti nel database, come tabelle, viste o indici. 

\textbf{Sintassi generale per una tabella:}
\begin{verbatim}
CREATE TABLE nome_tabella (
    col1 tipo_dato [constraint],
    col2 tipo_dato [constraint],
    ...
);
\end{verbatim}

\textbf{Esempio:}

\begin{verbatim}
CREATE TABLE Studente (
    Matricola INT PRIMARY KEY,
    Nome VARCHAR(50),
    Cognome VARCHAR(50),
    DataNascita DATE
);
\end{verbatim}

\textit{Descrizione:}  
\begin{itemize}
    \item \code{Matricola} è chiave primaria;
    \item \code{Nome} e \code{Cognome} sono stringhe fino a 50 caratteri;
    \item \code{DataNascita} è un campo di tipo data.
\end{itemize}

%==============================================================================
\subsection{Eliminare tabelle o altri oggetti con DROP}
%==============================================================================

Il comando \code{DROP} serve per eliminare completamente un oggetto del database, come una tabella. Tutti i dati e la struttura vengono cancellati. 

\textbf{Sintassi:}
\begin{verbatim}
DROP TABLE nome_tabella;
\end{verbatim}

\textbf{Esempio:}

\begin{verbatim}
DROP TABLE Studente;
\end{verbatim}

\textit{Attenzione:} questo comando è \textbf{irreversibile}.

%==============================================================================
\subsection{Modificare la struttura di una tabella tramite ALTER}
%==============================================================================

Il comando \code{ALTER} permette di modificare la struttura di una tabella già esistente, aggiungendo, modificando o eliminando colonne. 

\textbf{Sintassi generale:}
\begin{verbatim}
ALTER TABLE nome_tabella
ADD colonna tipo_dato;

ALTER TABLE nome_tabella
MODIFY colonna nuovo_tipo;

ALTER TABLE nome_tabella
DROP COLUMN colonna;
\end{verbatim}

\textbf{Esempi:}

\begin{verbatim}
-- Aggiungere una colonna
ALTER TABLE Studente
ADD Email VARCHAR(100);

-- Modificare il tipo di una colonna
ALTER TABLE Studente
MODIFY Cognome VARCHAR(100);

-- Eliminare una colonna
ALTER TABLE Studente
DROP COLUMN Email;
\end{verbatim}

%==============================================================================
\subsection{Vincoli di integrità tramite CONSTRAINTS}
%==============================================================================

I \textbf{constraints} sono regole applicate alle colonne delle tabelle per garantire l'\textbf{integrità dei dati}.  
Possono essere definite durante la creazione della tabella (\code{CREATE TABLE}) o modificate successivamente con \code{ALTER TABLE}. 

I principali tipi di vincoli sono:

\begin{itemize}
    \item \code{PRIMARY KEY} -- identifica univocamente ogni record nella tabella;
    \item \code{FOREIGN KEY} -- assicura la referenzialità tra tabelle;
    \item \code{UNIQUE} -- garantisce che tutti i valori di una colonna siano unici;
    \item \code{NOT NULL} -- impedisce valori nulli in una colonna;
    \item \code{CHECK} -- definisce una condizione che i valori della colonna devono rispettare.
\end{itemize}

%==============================================================================
\subsubsection{PRIMARY KEY}
%==============================================================================

Il vincolo \code{PRIMARY KEY} serve a identificare in modo univoco ogni record nella tabella. 

\textbf{Esempio:}

\begin{verbatim}
CREATE TABLE Studente (
    Matricola INT PRIMARY KEY,
    Nome VARCHAR(50),
    Cognome VARCHAR(50)
);
\end{verbatim}

\textit{Nota:} una tabella può avere solo una \code{PRIMARY KEY}, che può essere composta da una o più colonne (chiave primaria composta).

%==============================================================================
\subsubsection{FOREIGN KEY}
%==============================================================================

Il vincolo \code{FOREIGN KEY} serve a garantire la \textbf{referenzialità} tra due tabelle. 

\textbf{Esempio:}

\begin{verbatim}
CREATE TABLE Corso (
    CodiceMateria INT PRIMARY KEY,
    NomeMateria VARCHAR(100)
);

CREATE TABLE StudenteCorso (
    Matricola INT,
    CodiceMateria INT,
    FOREIGN KEY (Matricola) REFERENCES Studente(Matricola),
    FOREIGN KEY (CodiceMateria) REFERENCES Corso(CodiceMateria)
);
\end{verbatim}

\textit{Descrizione:}  
\begin{itemize}
    \item Ogni \code{Matricola} in \code{StudenteCorso} deve esistere nella tabella \code{Studente};
    \item Ogni \code{CodiceMateria} in \code{StudenteCorso} deve esistere nella tabella \code{Corso}.
\end{itemize}

%==============================================================================
\subsubsection{Altri vincoli comuni}
%==============================================================================

\begin{itemize}
    \item \code{UNIQUE}: garantisce valori unici in una colonna.
    \begin{verbatim}
    CREATE TABLE Studente (
        Email VARCHAR(100) UNIQUE
    );
    \end{verbatim}

    \item \code{NOT NULL}: obbliga ad inserire un valore nella colonna.
    \begin{verbatim}
    CREATE TABLE Studente (
        Nome VARCHAR(50) NOT NULL
    );
    \end{verbatim}

    \item \code{CHECK}: definisce una condizione sui valori della colonna.
    \begin{verbatim}
    CREATE TABLE Studente (
        Età INT CHECK (Età >= 18)
    );
    \end{verbatim}
\end{itemize}
%==============================================================================




\newpage
%==============================================================================
\section{DML -- Data Manipulation Language}
%==============================================================================

\textbf{DML} (Data Manipulation Language) è la parte di SQL che serve per \textbf{manipolare i dati} presenti nelle tabelle: inserire, aggiornare o cancellare record.  
I principali comandi DML sono:

\begin{itemize}
    \item \code{INSERT INTO} -- inserire nuovi record;
    \item \code{UPDATE} -- modificare record esistenti;
    \item \code{DELETE} -- eliminare record.
\end{itemize}

%==============================================================================
\subsection{Inserire nuovi dati con INSERT INTO}
%==============================================================================

Il comando \code{INSERT INTO} aggiunge nuove righe in una tabella. È possibile specificare le colonne in cui inserire i valori, oppure inserirli in tutte le colonne se vengono forniti in ordine. 

\textbf{Sintassi:}
\begin{verbatim}
INSERT INTO nome_tabella (col1, col2, ..., colN)
VALUES (val1, val2, ..., valN);
\end{verbatim}

\textbf{Esempio:}

\textit{Tabella \texttt{Libro} prima dell'inserimento:}
\begin{center}
\begin{tabular}{|c|c|c|}
\hline
Codice & Titolo & Autore \\
\hline
L01 & Il nome della rosa & Umberto Eco \\
L02 & Il fu Mattia Pascal & Luigi Pirandello \\
\hline
\end{tabular}
\end{center}

\begin{verbatim}
INSERT INTO Libro (Codice, Titolo, Autore)
VALUES ('L03', 'Se questo è un uomo', 'Primo Levi');
\end{verbatim}

\textit{Tabella dopo l'inserimento:}
\begin{center}
\begin{tabular}{|c|c|c|}
\hline
Codice & Titolo & Autore \\
\hline
L01 & Il nome della rosa & Umberto Eco \\
L02 & Il fu Mattia Pascal & Luigi Pirandello \\
L03 & Se questo è un uomo & Primo Levi \\
\hline
\end{tabular}
\end{center}

%==============================================================================
\subsection{Modificare dati esistenti tramite UPDATE}
%==============================================================================

Il comando \code{UPDATE} serve per aggiornare i valori di una o più colonne per le righe che soddisfano una condizione. È fondamentale usare \code{WHERE} per non modificare tutte le righe della tabella. 

\textbf{Sintassi:}
\begin{verbatim}
UPDATE nome_tabella
SET col1 = nuovo_valore1, col2 = nuovo_valore2, ...
WHERE condizione;
\end{verbatim}

\textbf{Esempio:}

\textit{Tabella \texttt{Libro} prima della modifica:}
\begin{center}
\begin{tabular}{|c|c|c|}
\hline
Codice & Titolo & Autore \\
\hline
L01 & Il nome della rosa & Umberto Eco \\
L02 & Il fu Mattia Pascal & Luigi Pirandello \\
L03 & Se questo è un uomo & Primo Levi \\
\hline
\end{tabular}
\end{center}

\begin{verbatim}
UPDATE Libro
SET Autore = 'Eco, Umberto'
WHERE Codice = 'L01';
\end{verbatim}

\textit{Tabella dopo la modifica:}
\begin{center}
\begin{tabular}{|c|c|c|}
\hline
Codice & Titolo & Autore \\
\hline
L01 & Il nome della rosa & Eco, Umberto \\
L02 & Il fu Mattia Pascal & Luigi Pirandello \\
L03 & Se questo è un uomo & Primo Levi \\
\hline
\end{tabular}
\end{center}

%==============================================================================
\subsection{Cancellare dati con DELETE}
%==============================================================================

Il comando \code{DELETE} rimuove le righe di una tabella che soddisfano una condizione. Senza \code{WHERE}, tutte le righe vengono eliminate. 

\textbf{Sintassi:}
\begin{verbatim}
DELETE FROM nome_tabella
WHERE condizione;
\end{verbatim}

\textbf{Esempio:}

\textit{Tabella \texttt{Libro} prima della cancellazione:}
\begin{center}
\begin{tabular}{|c|c|c|}
\hline
Codice & Titolo & Autore \\
\hline
L01 & Il nome della rosa & Eco, Umberto \\
L02 & Il fu Mattia Pascal & Luigi Pirandello \\
L03 & Se questo è un uomo & Primo Levi \\
\hline
\end{tabular}
\end{center}

\begin{verbatim}
DELETE FROM Libro
WHERE Codice = 'L02';
\end{verbatim}

\textit{Tabella dopo la cancellazione:}
\begin{center}
\begin{tabular}{|c|c|c|}
\hline
Codice & Titolo & Autore \\
\hline
L01 & Il nome della rosa & Eco, Umberto \\
L03 & Se questo è un uomo & Primo Levi \\
\hline
\end{tabular}
\end{center}

\bigskip
\textbf{Riepilogo DML:}
\begin{itemize}
    \item \code{INSERT INTO} aggiunge nuove righe;
    \item \code{UPDATE} modifica dati esistenti;
    \item \code{DELETE} rimuove righe dalla tabella.
\end{itemize}
%==============================================================================




%==============================================================================
\newpage
\section{SQL DQL (Data Query Language)}
%==============================================================================

\textbf{DQL} è la parte di SQL che serve per \textbf{interrogare} i dati, cioè leggere e visualizzare le informazioni presenti in una o più tabelle.  

\subsection{SELECT, FROM}
Il comando principale è \code{SELECT}, che permette di specificare quali colonne vogliamo visualizzare e quali righe includere nei risultati e \code{FROM}, che indica da quale tabella prendere i dati.\\\\
\textbf{Sintassi base di \code{SELECT}:}
\begin{verbatim}
SELECT lista_colonne
FROM nome_tabella;
\end{verbatim}
\textbf{Per selezionare tutte le colonne si usa l'asterisco \code{*}:}
\begin{verbatim}
SELECT * FROM nome_tabella;
\end{verbatim}
A partire dalla seguente tabella di esempio:
Tabella \texttt{Studente}:
\begin{center}
\begin{tabular}{|c|c|c|}
\hline
\textbf{Matricola} & \textbf{Nome} & \textbf{Cognome} \\
\hline
101 & Marco & Rossi \\
102 & Anna & Bianchi \\
103 & Luca & Verdi \\
\hline
\end{tabular}
\end{center}
\bigskip
\textbf{Esempio 1 -- Selezionare tutte le colonne}\\
Query:
\begin{verbatim}
SELECT * FROM Studente;
\end{verbatim}
\textit{Risultato:}
\begin{center}
\begin{tabular}{|c|c|c|}
\hline
Matricola & Nome & Cognome \\
\hline
101 & Marco & Rossi \\
102 & Anna & Bianchi \\
103 & Luca & Verdi \\
\hline
\end{tabular}
\end{center}
\bigskip
\textbf{Esempio 2 -- Selezionare solo alcune colonne}
\begin{verbatim}
SELECT Nome, Cognome FROM Studente;
\end{verbatim}
\textit{Risultato:}
\begin{center}
\begin{tabular}{|c|c|}
\hline
Nome & Cognome \\
\hline
Marco & Rossi \\
Anna & Bianchi \\
Luca & Verdi \\
\hline
\end{tabular}
\end{center}

\subsection{Filtrare i dati con la clausola \code{WHERE}}
La clausola \code{WHERE} serve per selezionare solo le righe che soddisfano una condizione.\\\\
\textbf{Sintassi:}
\begin{verbatim}
SELECT colonne
FROM tabella
WHERE condizione;
\end{verbatim}
\textbf{Esempio: Studenti con matricola maggiore di 101}
\begin{verbatim}
SELECT * FROM Studente
WHERE Matricola > 101;
\end{verbatim}

\textit{Risultato:}
\begin{center}
\begin{tabular}{|c|c|c|}
\hline
Matricola & Nome & Cognome \\
\hline
102 & Anna & Bianchi \\
103 & Luca & Verdi \\
\hline
\end{tabular}
\end{center}

\subsubsection{Gli operatori logici AND, OR, NOT}
Per combinare più condizioni si usano:

\begin{itemize}
    \item \textbf{AND} -- tutte le condizioni devono essere vere.
    \item \textbf{OR} -- almeno una condizione deve essere vera.
    \item \textbf{NOT} -- la condizione non deve essere vera.
\end{itemize}
\textbf{Esempi:}
\begin{itemize}
    \item AND:
    \begin{verbatim}
    SELECT * FROM Studente
    WHERE Matricola > 101 AND Cognome = 'Verdi';
    \end{verbatim}

    \item OR:
    \begin{verbatim}
    SELECT * FROM Studente
    WHERE Cognome = 'Rossi' OR Cognome = 'Bianchi';
    \end{verbatim}

    \item NOT:
    \begin{verbatim}
    SELECT * FROM Studente
    WHERE NOT Cognome = 'Rossi';
    \end{verbatim}
\end{itemize}

\subsection{Ordinare i risultati con \code{ORDER BY}}
Dopo aver selezionato e filtrato i dati, possiamo ordinarli con la clausola \code{ORDER BY}.

\textbf{Sintassi:}
\begin{verbatim}
SELECT colonne
FROM tabella
ORDER BY colonna1 [ASC|DESC], colonna2 [ASC|DESC];
\end{verbatim}

\textbf{Esempio 1 -- Ordinare studenti per cognome crescente:}
\begin{verbatim}
SELECT * FROM Studente
ORDER BY Cognome;
\end{verbatim}

\textit{Risultato:}
\begin{center}
\begin{tabular}{|c|c|c|}
\hline
Matricola & Nome & Cognome \\
\hline
102 & Anna & Bianchi \\
101 & Marco & Rossi \\
103 & Luca & Verdi \\
\hline
\end{tabular}
\end{center}

\textbf{Esempio 2 -- Ordinamento decrescente:}
\begin{verbatim}
SELECT * FROM Studente
ORDER BY Cognome DESC;
\end{verbatim}

\textbf{Esempio 3 -- Ordinare per più colonne:}
\begin{verbatim}
SELECT * FROM Studente
ORDER BY Cognome ASC, Nome ASC;
\end{verbatim}

\medskip
\textbf{Nota:} È possibile combinare \code{WHERE} e \code{ORDER BY}:
\begin{verbatim}
SELECT * FROM Studente
WHERE Matricola > 101
ORDER BY Cognome;
\end{verbatim}

\bigskip
\textbf{Riepilogo:}
\begin{itemize}
    \item \code{SELECT} permette di scegliere le colonne da visualizzare.
    \item \code{WHERE} filtra le righe.
    \item \code{AND}, \code{OR}, \code{NOT} permettono condizioni complesse.
    \item \code{ORDER BY} ordina i risultati.
\end{itemize}



%==============================================================================
\subsection{GROUP BY e gli operatori di aggregazione in SQL}
%==============================================================================

Quando si vogliono analizzare i dati raggruppandoli per valori comuni di una o più colonne, si utilizza la clausola \code{GROUP BY}.  
Le funzioni di aggregazione permettono di calcolare valori riassuntivi su gruppi di dati, come conteggi, somme, medie, minimi e massimi.

\bigskip
\textbf{Funzioni di aggregazione principali:}
\begin{itemize}
    \item \code{COUNT(colonna)} -- conta il numero di valori presenti.
    \item \code{SUM(colonna)} -- somma dei valori numerici.
    \item \code{AVG(colonna)} -- media dei valori numerici.
    \item \code{MIN(colonna)} -- valore minimo.
    \item \code{MAX(colonna)} -- valore massimo.
\end{itemize}

\subsubsection*{Esempio pratico: Ordini dei clienti}

Tabella \texttt{Ordine}:
\begin{center}
\begin{tabular}{|c|c|c|c|}
\hline
\textbf{NumeroOrdine} & \textbf{Data} & \textbf{Cliente} & \textbf{Importo} \\
\hline
5001 & 2024-06-01 & Rossi & 120.00 \\
5002 & 2024-06-02 & Bianchi & 85.50 \\
5003 & 2024-06-02 & Neri & 42.00 \\
5004 & 2024-06-03 & Rossi & 75.00 \\
5005 & 2024-06-03 & Bianchi & 100.00 \\
\hline
\end{tabular}
\end{center}

\subsubsection{COUNT -- Contare gli ordini per cliente}
\begin{verbatim}
SELECT Cliente, COUNT(*) AS NumeroOrdini
FROM Ordine
GROUP BY Cliente;
\end{verbatim}

\textit{Risultato:}
\begin{center}
\begin{tabular}{|c|c|}
\hline
Cliente & NumeroOrdini \\
\hline
Rossi & 2 \\
Bianchi & 2 \\
Neri & 1 \\
\hline
\end{tabular}
\end{center}

\subsubsection{SUM -- Sommare l'importo totale per cliente}
\begin{verbatim}
SELECT Cliente, SUM(Importo) AS TotaleSpeso
FROM Ordine
GROUP BY Cliente;
\end{verbatim}

\textit{Risultato:}
\begin{center}
\begin{tabular}{|c|c|}
\hline
Cliente & TotaleSpeso \\
\hline
Rossi & 195.00 \\
Bianchi & 185.50 \\
Neri & 42.00 \\
\hline
\end{tabular}
\end{center}

\subsubsection{AVG -- Calcolare la media degli importi per cliente}
\begin{verbatim}
SELECT Cliente, AVG(Importo) AS MediaImporto
FROM Ordine
GROUP BY Cliente;
\end{verbatim}

\textit{Risultato:}
\begin{center}
\begin{tabular}{|c|c|}
\hline
Cliente & MediaImporto \\
\hline
Rossi & 97.50 \\
Bianchi & 92.75 \\
Neri & 42.00 \\
\hline
\end{tabular}
\end{center}

\subsubsection{MIN e MAX -- Importo minimo e massimo per cliente}
\begin{verbatim}
SELECT Cliente, MIN(Importo) AS MinImporto, MAX(Importo) AS MaxImporto
FROM Ordine
GROUP BY Cliente;
\end{verbatim}

\textit{Risultato:}
\begin{center}
\begin{tabular}{|c|c|c|}
\hline
Cliente & MinImporto & MaxImporto \\
\hline
Rossi & 75.00 & 120.00 \\
Bianchi & 85.50 & 100.00 \\
Neri & 42.00 & 42.00 \\
\hline
\end{tabular}
\end{center}

\subsection{Combinare filtri con \code{HAVING}}
Per filtrare i gruppi dopo il raggruppamento, si utilizza la clausola \code{HAVING} (simile a \code{WHERE}, ma agisce sui gruppi).

\textbf{Esempio: clienti con più di un ordine}
\begin{verbatim}
SELECT Cliente, COUNT(*) AS NumeroOrdini
FROM Ordine
GROUP BY Cliente
HAVING COUNT(*) > 1;
\end{verbatim}

\textit{Risultato:}
\begin{center}
\begin{tabular}{|c|c|}
\hline
Cliente & NumeroOrdini \\
\hline
Rossi & 2 \\
Bianchi & 2 \\
\hline
\end{tabular}
\end{center}

\bigskip
\textbf{Riepilogo:}
\begin{itemize}
    \item \code{GROUP BY} raggruppa le righe in base ai valori di una o più colonne.
    \item Funzioni di aggregazione (\code{COUNT, SUM, AVG, MIN, MAX}) calcolano valori riassuntivi per ciascun gruppo.
    \item \code{HAVING} filtra i gruppi dopo l’aggregazione.
\end{itemize}















%==============================================================================
\subsection{Le JOIN in SQL}
%==============================================================================

Quando i dati sono distribuiti su più tabelle collegate tra loro, spesso è necessario \textbf{unire le informazioni} per ottenere risultati complessi.  
In SQL questo si fa con le \textbf{JOIN}, che permettono di combinare i dati di due o più tabelle sulla base di una relazione tra le colonne, tipicamente una chiave primaria e una chiave esterna.

\bigskip
\textbf{Regole fondamentali per una JOIN:}
\begin{itemize}
    \item \textbf{Tipo di JOIN:} indica quali righe vengono restituite dal risultato.
    \item \textbf{Condizione di JOIN:} specificata con \code{ON}, indica come le tabelle devono essere collegate.
\end{itemize}

\subsubsection{Tipi principali di JOIN}

\begin{itemize}
    \item \textbf{INNER JOIN:} restituisce solo le righe che hanno corrispondenza in entrambe le tabelle.
    \item \textbf{LEFT JOIN (o LEFT OUTER JOIN):} restituisce tutte le righe della tabella di sinistra e, se presenti, i dati corrispondenti della tabella di destra; se non c'è corrispondenza, i valori della destra sono NULL.
    \item \textbf{FULL OUTER JOIN:} restituisce tutte le righe di entrambe le tabelle; dove non c'è corrispondenza, i valori mancanti sono NULL.
\end{itemize}

\subsubsection*{Esempio pratico}

Supponiamo di avere le seguenti tabelle:

\textbf{Studente}
\begin{center}
\begin{tabular}{|c|c|c|}
\hline
\textbf{Matricola} & \textbf{Nome} & \textbf{Cognome} \\
\hline
101 & Marco & Rossi \\
102 & Anna & Bianchi \\
103 & Luca & Verdi \\
104 & Sara & Neri \\
\hline
\end{tabular}
\end{center}

\textbf{Voto}
\begin{center}
\begin{tabular}{|c|c|c|}
\hline
\textbf{Matricola} & \textbf{CodiceMateria} & \textbf{Voto} \\
\hline
101 & MAT01 & 8 \\
102 & INF02 & 7 \\
103 & FIS03 & 9 \\
101 & INF02 & 9 \\
\hline
\end{tabular}
\end{center}

\textbf{Materia}
\begin{center}
\begin{tabular}{|c|c|}
\hline
\textbf{CodiceMateria} & \textbf{NomeMateria} \\
\hline
MAT01 & Matematica \\
INF02 & Informatica \\
FIS03 & Fisica \\
\hline
\end{tabular}
\end{center}

\subsubsection{INNER JOIN}
Restituisce solo gli studenti che hanno almeno un voto, mostrando anche la materia.

\begin{verbatim}
SELECT Studente.Nome, Studente.Cognome,
       Materia.NomeMateria, Voto.Voto
FROM Studente
INNER JOIN Voto ON Studente.Matricola = Voto.Matricola
INNER JOIN Materia ON Voto.CodiceMateria = Materia.CodiceMateria;
\end{verbatim}

\textit{Risultato:}
\begin{center}
\begin{tabular}{|c|c|c|c|}
\hline
Nome & Cognome & NomeMateria & Voto \\
\hline
Marco & Rossi & Matematica & 8 \\
Marco & Rossi & Informatica & 9 \\
Anna & Bianchi & Informatica & 7 \\
Luca & Verdi & Fisica & 9 \\
\hline
\end{tabular}
\end{center}

\subsubsection{LEFT JOIN}
Mostra tutti gli studenti, anche quelli che non hanno voti. Dove mancano dati, i valori sono NULL.

\begin{verbatim}
SELECT Studente.Nome, Studente.Cognome,
       Materia.NomeMateria, Voto.Voto
FROM Studente
LEFT JOIN Voto ON Studente.Matricola = Voto.Matricola
LEFT JOIN Materia ON Voto.CodiceMateria = Materia.CodiceMateria;
\end{verbatim}

\textit{Risultato:}
\begin{center}
\begin{tabular}{|c|c|c|c|}
\hline
Nome & Cognome & NomeMateria & Voto \\
\hline
Marco & Rossi & Matematica & 8 \\
Marco & Rossi & Informatica & 9 \\
Anna & Bianchi & Informatica & 7 \\
Luca & Verdi & Fisica & 9 \\
Sara & Neri & NULL & NULL \\
\hline
\end{tabular}
\end{center}

\subsubsection{FULL OUTER JOIN}
Restituisce tutte le righe di entrambe le tabelle, mostrando anche eventuali dati non associati.

\begin{verbatim}
SELECT Studente.Nome, Studente.Cognome,
       Materia.NomeMateria, Voto.Voto
FROM Studente
FULL OUTER JOIN Voto ON Studente.Matricola = Voto.Matricola
FULL OUTER JOIN Materia ON Voto.CodiceMateria = Materia.CodiceMateria;
\end{verbatim}

\textit{Risultato:}
\begin{center}
\begin{tabular}{|c|c|c|c|}
\hline
Nome & Cognome & NomeMateria & Voto \\
\hline
Marco & Rossi & Matematica & 8 \\
Marco & Rossi & Informatica & 9 \\
Anna & Bianchi & Informatica & 7 \\
Luca & Verdi & Fisica & 9 \\
Sara & Neri & NULL & NULL \\
\hline
\end{tabular}
\end{center}

\bigskip
\textbf{Riepilogo sulle JOIN:}
\begin{itemize}
    \item \code{INNER JOIN}: restituisce solo le corrispondenze tra le tabelle.
    \item \code{LEFT JOIN}: restituisce tutte le righe della tabella di sinistra, anche senza corrispondenza.
    \item \code{FULL OUTER JOIN}: restituisce tutte le righe di entrambe le tabelle, anche senza corrispondenza.
    \item Sempre specificare la \code{condizione di JOIN} con \code{ON} per evitare il prodotto cartesiano.
\end{itemize}












%====================================================================
\newpage
\section{Progettazione di una base di dati}
  ...
  \subsection{Analisi dei requisiti}
  \subsection{Progettazione concettuale}
      Qui puoi inserire il dettaglio con:
      - Modello Entità-Relazioni (ER)
      - Tipi di entità e attributi
      - Relazioni tra entità e cardinalità
      - Identificazione delle chiavi primarie e opzionali
      - Eventuali vincoli di integrità concettuali
      - Disegni ER come esempi
  \subsection{Progettazione logica}
  \subsection{Progettazione fisica}

\newpage

% ==========================
%         ESERCIZI
% ==========================
\thispagestyle{empty}
\begin{center}
    \vspace*{5cm}
    {\Huge\bfseries Esercizi}
\end{center}
\newpage


% ==========================
\section{Esercizi -- Modello relazionale}
% ==========================

\begin{enumerate}

\item 

Definire lo schema della base di dati riportata sotto. Come sono chiamati \textit{Codice, Autore, ecc}?. Quante istanze sono rappresentate?

\begin{center}
\begin{tabular}{|c|c|c|c|c|c|}
\hline
\textbf{Codice} & \textbf{Autore} & \textbf{Conferenza} & \textbf{Sito} & \textbf{Anno} & \textbf{Pagine} \\
\hline
101212 & Marco & ICC & Bologna & 2012 & 7 \\
101214 & Matteo & INFOCOM & Las Vegas & 2011 & 8 \\
11212 & Luca & INFOCOM & Las Vegas & 2011 & 8 \\
11212 & Luigi & INFOCOM & Las Vegas & 2011 & 8 \\
101214 & Matteo & INFOCOM & Las Vegas & 2011 & 8 \\
101244 & Luca & MOBICOM & NY & 2008 & 10 \\
101233 & Carlo & DYSPAN & Londra & 2012 & 5 \\
\hline
\end{tabular}
\end{center}


\item Indicare se la seguente relazione costituisce una \textbf{relazione valida} nel modello relazionale. Se non lo è, spiegare quali problemi presenta.

\begin{center}
\begin{tabular}{|c|c|c|c|c|c|}
\hline
\textbf{Codice} & \textbf{Autore} & \textbf{Conferenza} & \textbf{Sito} & \textbf{Anno} & \textbf{Pagine} \\
\hline
101212 & Marco & ICC & Bologna & 2012 & 7 \\
101214 & Matteo & INFOCOM & Las Vegas & 2011 & 8 \\
11212 & Luca & INFOCOM & Las Vegas & 2011 & 8 \\
11212 & Luigi & INFOCOM & Las Vegas & 2011 & 8 \\
101214 & Matteo & INFOCOM & Las Vegas & 2011 & 8 \\
101244 & Luca & MOBICOM & NY & 2008 & 10 \\
101233 & Carlo & DYSPAN & Londra & 2012 & 5 \\
\hline
\end{tabular}
\end{center}


\item 

Indicare se la seguente tabella costituisce una \textbf{relazione valida} nel modello relazionale.  
Se non lo è, spiegare quali problemi presenta.

\begin{center}
\begin{tabular}{|c|c|c|c|c|c|}
\hline
\textbf{Codice} & \textbf{Autore} & \textbf{Conferenza} & \textbf{Sito} & \textbf{Anno} & \textbf{Pagine} \\
\hline
101212 & Marco & ICC & Bologna & 2012 & 7 \\
101214 & Matteo & INFOCOM & Las Vegas & 2011 & 8 \\
11212 & Luca & INFOCOM & Las Vegas & 2011 & 8 \\
11212 & Luigi & INFOCOM & Las Vegas & 2011 & 8 \\
 & Matteo & INFOCOM & Las Vegas & 2011 & 8 \\
101244 & Luca & MOBICOM & NY & 2008 & 10 \\
101233 & Carlo & DYSPAN & Londra & 2012 & 5 \\
\hline
\end{tabular}
\end{center}


\item 

Indicare se la seguente tabella costituisce una \textbf{relazione valida} nel modello relazionale.  
Se non lo è, spiegare quali problemi presenta.

\begin{center}
\begin{tabular}{|c|c|c|c|c|c|}
\hline
\textbf{Codice} & \textbf{Autore} & \textbf{Conferenza} & \textbf{Sito} & \textbf{Anno} & \textbf{Pagine} \\
\hline
101212 & Marco & ICC & Bologna & 2012 & 7 \\
 & Matteo & INFOCOM & Las Vegas & 2011 & 8 \\
11212 & Luca & INFOCOM & Las Vegas & 2011 & 8 \\
11212 & Luigi & INFOCOM & Las Vegas & 2011 & 8 \\
 & Matteo & INFOCOM & Las Vegas & 2011 & 8 \\
101244 & Luca & MOBICOM & NY & 2008 & 10 \\
101233 & Carlo & DYSPAN & Londra & 2012 & 5 \\
\hline
\end{tabular}
\end{center}


\item 

Scrivere lo schema di una relazione \texttt{Cliente}, descritto con i seguenti attributi:
\begin{itemize}
    \item CodiceCliente
    \item Nome
    \item Cognome
    \item DataNascita
    \item Email
\end{itemize}

Successivamente, inserire 4 istanze di esempio con dati realistici.  
Indicare quali valori potrebbero violare il vincolo di dominio per il campo DataNascita

\vspace{0.5cm}

\item
Si desidera modellare una \textbf{prenotazione in un ristorante}. Ogni prenotazione registra:
\begin{itemize}
    \item La data e l’ora della prenotazione
    \item Il numero del tavolo
    \item Il cliente che ha prenotato
\end{itemize}

Scrivere lo schema della relazione e inserire 4 tuple di esempio. Indicare per ogni attributo i vincoli di dominio e di non nullità.

\vspace{0.5cm}

\item 

Si vuole rappresentare un registro di libri in una biblioteca. Ogni libro è descritto da un codice ISBN, un titolo, un autore, l’anno di pubblicazione e il genere letterario.  

Scrivere lo schema della relazione e inserire 4 tuple di esempio. Indicare per ogni attributo i vincoli di dominio e di non nullità.

\vspace{0.5cm}  


\item
Si desidera rappresentare il \textbf{catalogo prodotti} di un e-commerce. Ogni prodotto è descritto da un codice, un nome, una categoria, un prezzo e una quantità disponibile in magazzino.

Scrivere lo schema della relazione e 4 istanze d'esempio. Indicare per ogni attributo i vincoli di dominio e di non nullità.

%\vspace{0.5cm}
%
%\item
%Si vuole gestire un **registro libri** di una biblioteca. Ogni libro ha:
%- un codice ISBN,
%- un titolo,
%- un autore,
%- l’anno di pubblicazione,
%- il genere letterario.
%
%\textbf{Compito:}  
%Scrivere lo schema della relazione Libro con gli attributi e i vincoli di dominio (ad esempio %anno di pubblicazione positivo, ISBN numerico se richiesto). Inserire 5 tuple di esempio e %indicare eventuali valori non validi.
%
%\vspace{0.5cm}
%
%\item
%Si vuole rappresentare un **ordine effettuato da un cliente**. Ogni ordine ha:
%- un numero identificativo ordine,
%- la data in cui è stato effettuato,
%- il cliente che lo ha effettuato,
%- l’importo totale dell’ordine.
%
%\textbf{Compito:}  
%Scrivere lo schema della relazione Ordine, indicare i vincoli di dominio (ad esempio importo %positivo, data valida) e quali attributi non possono essere nulli. Inserire 4 tuple di esempio.
%
%
%\vspace{0.5cm}
%
%\item
%Si vuole gestire le **iscrizioni ai corsi di una scuola**. Ogni iscrizione registra:
%- lo studente,
%- il corso a cui si iscrive,
%- la data di iscrizione.
%
%\textbf{Compito:}  
%Scrivere lo schema della relazione IscrizioneCorso, indicare i vincoli di dominio e di non %null. Inserire almeno 4 tuple di esempio.


\newpage
\item 

Considera la seguente tabella \texttt{Libro}:
\begin{center}
\begin{tabular}{|c|c|c|c|}
\hline
\textbf{Codice} & \textbf{Titolo} & \textbf{Autore} & \textbf{Anno} \\
\hline
L01 & Il nome della rosa & Umberto Eco & 1980 \\
L02 & Il fu Mattia Pascal & Luigi Pirandello & 1904 \\
L03 & Se questo è un uomo & Primo Levi & 1947 \\
L04 & Il barone rampante & Italo Calvino & 1957 \\
\hline
\end{tabular}
\end{center}

\begin{enumerate}
    \item Quale attributo può essere scelto come chiave primaria? Motiva la risposta.
    \item Esistono superchiavi diverse dalla chiave primaria? Fai almeno un esempio.
    \item È possibile che la combinazione \{\texttt{Titolo}, \texttt{Autore}\} sia una chiave primaria? Quando?
\end{enumerate}

\vspace{0.5cm}

% Esercizio 2 fornito dall'utente
\item 

Considera la seguente relazione \texttt{Prenotazione}:
\begin{center}
\begin{tabular}{|c|c|c|c|}
\hline
\textbf{IDPrenotazione} & \textbf{Data} & \textbf{Ora} & \textbf{Tavolo} \\
\hline
1 & 2024-05-10 & 20:00 & 12 \\
2 & 2024-05-10 & 21:00 & 7 \\
3 & 2024-05-11 & 19:30 & 5 \\
4 & 2024-05-11 & 21:00 & 12 \\
\hline
\end{tabular}
\end{center}

\begin{enumerate}
    \item Indica una possibile chiave primaria per la relazione.
    \item La combinazione \{\texttt{Data}, \texttt{Ora}, \texttt{Tavolo}\} può essere una chiave candidata? Motiva la risposta.
    \item Esistono attributi che possono assumere valori nulli? Spiega.
\end{enumerate}

\vspace{0.5cm}

% Esercizio originale 5
\item 

Considera la tabella \texttt{Ordine}:
\begin{center}
\begin{tabular}{|c|c|c|c|}
\hline
\textbf{NumeroOrdine} & \textbf{Data} & \textbf{Cliente} & \textbf{Importo} \\
\hline
5001 & 2024-06-01 & Rossi & 120.00 \\
5002 & 2024-06-02 & Bianchi & 85.50 \\
5003 & 2024-06-02 & Neri & 42.00 \\
5004 & 2024-06-03 & Rossi & 75.00 \\
\hline
\end{tabular}
\end{center}

\begin{enumerate}
    \item Qual è la chiave primaria più naturale per questa relazione?
    \item La coppia \{\texttt{Data}, \texttt{Cliente}\} può essere una chiave? Giustifica.
    \item Se possono esistere più ordini nello stesso giorno per lo stesso cliente, quali attributi dovresti usare per identificare univocamente ogni ordine?
\end{enumerate}

\vspace{0.5cm}

% Esercizio originale 6
\item 

Osserva la seguente tabella \texttt{PrenotazioneHotel}:
\begin{center}
\begin{tabular}{|c|c|c|c|c|}
\hline
\textbf{IDPrenotazione} & \textbf{Cliente} & \textbf{Camera} & \textbf{DataInizio} & \textbf{DataFine} \\
\hline
10 & Verdi & 201 & 2024-07-01 & 2024-07-05 \\
11 & Bianchi & 105 & 2024-07-02 & 2024-07-03 \\
12 & Neri & 201 & 2024-07-06 & 2024-07-10 \\
13 & Verdi & 105 & 2024-07-10 & 2024-07-12 \\
\hline
\end{tabular}
\end{center}

\begin{enumerate}
    \item Quale attributo può essere scelto come chiave primaria? Esistono altre possibilità?
    \item La combinazione \{\texttt{Camera}, \texttt{DataInizio}\} può essere una chiave? In quali condizioni?
    \item L’attributo \texttt{Cliente} può essere chiave? Perché?
\end{enumerate}

\vspace{0.5cm}

% Esercizio originale 7
\item 

Considera la tabella \texttt{IscrizioneCorso}:
\begin{center}
\begin{tabular}{|c|c|c|}
\hline
\textbf{Matricola} & \textbf{CodiceCorso} & \textbf{DataIscrizione} \\
\hline
201 & INF01 & 2024-09-01 \\
202 & MAT02 & 2024-09-01 \\
201 & MAT02 & 2024-09-02 \\
203 & INF01 & 2024-09-03 \\
\hline
\end{tabular}
\end{center}

\begin{enumerate}
    \item Quale insieme di attributi può essere scelto come chiave primaria?
    \item Esistono altre superchiavi? Forniscine almeno una.
    \item L’attributo \texttt{DataIscrizione} può essere chiave? Perché?
\end{enumerate}


\item 

Dato lo schema relazionale seguente:

\begin{itemize}
    \item \texttt{AGENZIA(Nome, Indirizzo, Telefono)}
    \item \texttt{CASA(Indirizzo, NrInterno, CodProprietario, Prezzo)}
    \item \texttt{INSERZIONI(Codice, IndirizzoCasa, InternoCasa, NomeAgenzia, DataInserzione)}
    \item \texttt{PROPRIETARIO(Codice, Nome, Cognome, Telefono)}
\end{itemize}

\textbf{Richiesta:} Individuare i vincoli di integrità referenziale presenti tra le relazioni.


\item A partire dall'esercizio precedente, descrivere a parola cosa modella lo schema relazionale rappresentato, generando un documento di specifiche.


\item \textbf{Tradurre nel modello relazionale il seguente documento di
specifiche.}

Si vuole progettare una base di dati per la gestione di un cinema.
Si vogliono registrare i film, con titolo, genere e durata.
Si vogliono gestire le sale, con nome e numero di posti.
Si vuole tenere traccia delle proiezioni: quale film viene proiettato in quale sala e a che orario.
Infine, si vogliono registrare i biglietti venduti, con i dati del cliente e la proiezione a cui il biglietto si riferisce.


\item \textbf{Tradurre nel modello relazionale il seguente documento di specifiche.}

Ogni utente dispone di email, password, nome, cognome, data di nascita. Ogni utente segue/è follower di altro utenti e ha dei follower a sua volta. Ogni utente dispone di una propria bacheca di messaggi ricevuti. Ogni messaggio dispone di: codice, data di invio, testo, e mittente (il mittente è un utente della piattaforma). Ogni messaggio è destinato ad un solo utente.


\item \textbf{Tradurre nel modello relazionale il seguente documento di specifiche.}

Si vuole progettare una base di dati per la gestione dei circoli tennis presenti in città.
Ogni circolo è caratterizzato da un nome univoco, indirizzo, recapito telefonico ed email.
Si vogliono gestire le informazioni relative agli utenti: ogni utente ha codice fiscale, nome, cognome, data e luogo di nascita e una lista di recapiti telefonici.
Ogni utente è iscritto a un singolo circolo tramite tessera associativa annuale, ma può anche frequentare altri circoli a cui non è iscritto.
Si vuole tenere traccia di queste frequentazioni, sapendo che un utente può frequentare più circoli (potenzialmente tutti).

\item \textbf{Tradurre nel modello relazionale il seguente documento di specifiche.}

Si vuole progettare una base di dati per la gestione dei treni e dei biglietti ferroviari.

Ogni treno è identificato da un codice univoco.
Il codice del treno è composto da una sigla di due lettere che identifica la tipologia (AV per Alta Velocità, RE per Regionale), seguita da quattro cifre numeriche. 
Esempi di codici validi sono AV1023, RE0456.
Ogni treno è inoltre caratterizzato da numero massimo di posti e anno di costruzione.

Ogni tratta è identificata da un codice univoco ed è caratterizzata da stazione di partenza, stazione di arrivo e distanza in chilometri.

Le tratte sono indipendenti dai treni.
Un treno può effettuare corse su tratte differenti.

Una corsa rappresenta l’effettivo viaggio di un treno su una determinata tratta in una specifica data.
Per ogni corsa si vogliono memorizzare:
data, orario di partenza e orario di arrivo.

Si tenga conto che:
\begin{itemize}
    \item ogni corsa è effettuata da un solo treno;
    \item ogni corsa è relativa a una sola tratta;
    \item un treno può effettuare molte corse;
    \item una tratta può essere percorsa da molti treni in corse diverse.
\end{itemize}

\item \textbf{Estensione del modello: gestione delle fermate intermedie}

Nel modello precedente una tratta è stata descritta unicamente tramite stazione di partenza e stazione di arrivo. Si vuole ora estendere la base di dati per rappresentare tratte composte da più fermate intermedie.

Ogni tratta può essere composta da più fermate, ciascuna associata a una stazione.
Le stazioni sono identificate da un codice univoco e caratterizzate da nome e città.

Per ogni fermata si vuole memorizzare:
\begin{itemize}
    \item la stazione;
    \item l'ordine progressivo lungo la tratta.
\end{itemize}

Si richiede di modificare il modello relazionale introducendo le nuove strutture necessarie,
mantenendo la coerenza con le entità già definite (Treno, Tratta, Corsa).

\end{enumerate}


\newpage
\section{Esercizi - SQL DDL}

I seguenti esercizi hanno l'obiettivo di scrivere le istruzioni SQL DDL a partire dagli schemi relazionali forniti. Per ciascun esercizio è necessario:
\begin{itemize}
    \item seguire fedelmente lo schema relazionale;
    \item dove non presenti, aggiungere ID\_ come chiavi surrogate
    \item definire le \textbf{PRIMARY KEY} e i vincoli di dominio (\textbf{NOT NULL}, \textbf{UNIQUE}, \textbf{CHECK});
    \item individuare e dichiarare le \textbf{FOREIGN KEY};
    \item utilizzare nomi coerenti con la \textbf{naming convention} accordata;
    \item creare campi \textbf{parlanti}, chiari e descrittivi del loro contenuto.
\end{itemize}
La valutazione considererà sia la correttezza tecnica delle DDL sia la chiarezza e leggibilità dei nomi e dei campi.

\begin{enumerate}

%====================================================
\item \textbf{Gestione Palestra}

A partire dal seguente schema relazionale, scrivere le istruzioni SQL DDL.
Le istruzioni devono comprendere istruzioni di \textbf{CREATE TABLE}, definizione delle \textbf{primary key} e \textbf{foreign key} (da individuare). Specificare inoltre i vincoli di domininio intra-relazionali \textbf{NOT NULL}, \textbf{UNIQUE} e \textbf{CHECK}.

\textbf{Schema relazionale}
\begin{itemize}
    \item \texttt{CLIENTE(\underline{CodiceFiscale}, Nome, Cognome, DataNascita, Email, Telefono, Indirizzo, Citta, CAP, DataIscrizione)}
    \item \texttt{ISTRUTTORE(\underline{Matricola}, Nome, Cognome, Specializzazione, Email, Telefono, Stipendio, DataAssunzione, Livello, Certificazione)}
    \item \texttt{CORSO(\underline{Codice}, Nome, Descrizione, DurataMinuti, Livello, MaxPartecipanti, Sala, GiornoSettimana, OrarioInizio, MatricolaIstruttore)}
    \item \texttt{ISCRIZIONE(\underline{CodiceFiscale}, \underline{CodiceCorso}, \underline{DataIscrizione}, StatoPagamento, PrezzoPagato, MetodoPagamento, DataScadenza, FrequenzaSettimanale, Sconto, Attivo)}
\end{itemize}

\textbf{Vincoli di dominio}
\begin{itemize}
\item Email univoca.
\item DataNascita con età >= 16.
\item Stipendio > 1000.
\item DurataMinuti > 0.
\item MaxPartecipanti > 0.
\item PrezzoPagato >= 0.
\item Sconto tra 0 e 100.
\end{itemize}
\newpage



%====================================================
\item \textbf{E-commerce}

A partire dal seguente schema relazionale, scrivere le istruzioni SQL DDL.
Le istruzioni devono comprendere istruzioni di \textbf{CREATE TABLE}, definizione delle \textbf{primary key} e \textbf{foreign key} (da individuare). Specificare inoltre i vincoli di dominio intra-relazionali \textbf{NOT NULL}, \textbf{UNIQUE} e \textbf{CHECK}.

\textbf{Schema relazionale}
\begin{itemize}
    \item \texttt{CLIENTE(\underline{ID}, Nome, Cognome, Email, Telefono, Password, Indirizzo, Citta, CAP, DataRegistrazione)}
    \item \texttt{PRODOTTO(\underline{Codice}, Nome, Descrizione, Prezzo, QuantitaMagazzino, Categoria, Peso, Marca, DataInserimento, Attivo)}
    \item \texttt{ORDINE(\underline{ID}, DataOrdine, Stato, Totale, MetodoPagamento, IndirizzoSpedizione, CittaSpedizione, CAPSpedizione, IDCliente, DataSpedizione)}
    \item \texttt{DETTAGLIO\_ORDINE(\underline{IDOrdine}, \underline{CodiceProdotto}, Quantita, PrezzoUnitario, ScontoPercentuale, IVA, TotaleRiga, StatoRiga, DataConsegnaPrevista, Reso)}
\end{itemize}

\textbf{Vincoli di dominio}
\begin{itemize}
\item Email univoca.
\item Prezzo > 0.
\item QuantitaMagazzino >= 0.
\item Quantita > 0.
\item ScontoPercentuale tra 0 e 100.
\item IVA tra 0 e 100.
\item Totale >= 0.
\end{itemize}
\newpage





%====================================================
\item \textbf{Sistema Ospedaliero}

A partire dal seguente schema relazionale, scrivere le istruzioni SQL DDL.
Le istruzioni devono comprendere istruzioni di \textbf{CREATE TABLE}, definizione delle \textbf{primary key} e \textbf{foreign key} (da individuare). Specificare inoltre i vincoli di dominio intra-relazionali \textbf{NOT NULL}, \textbf{UNIQUE} e \textbf{CHECK}.

\textbf{Schema relazionale}
\begin{itemize}
    \item \texttt{PAZIENTE(\underline{CodiceFiscale}, Nome, Cognome, DataNascita, Email, Telefono, Indirizzo, Citta, CAP, GruppoSanguigno)}
    \item \texttt{MEDICO(\underline{Matricola}, Nome, Cognome, Specializzazione, Email, Telefono, Reparto, Stipendio, DataAssunzione, Livello)}
    \item \texttt{REPARTO(\underline{Codice}, Nome, Piano, NumeroPostiLetto, Telefono, Responsabile, Edificio, BudgetAnnuale, Tipo, Attivo)}
    \item \texttt{RICOVERO(\underline{ID}, DataIngresso, DataDimissione, Motivo, Diagnosi, \\CodiceFiscalePaziente, MatricolaMedico, CodiceReparto, Stato, CostoTotale)}
\end{itemize}

\textbf{Vincoli di dominio}
\begin{itemize}
\item Email univoca.
\item Stipendio > 1500.
\item NumeroPostiLetto > 0.
\item BudgetAnnuale >= 0.
\item DataDimissione >= DataIngresso.
\item CostoTotale >= 0.
\end{itemize}
\newpage

%====================================================
\item \textbf{Gestione Azienda}

A partire dal seguente schema relazionale, scrivere le istruzioni SQL DDL.
Le istruzioni devono comprendere istruzioni di \textbf{CREATE TABLE}, definizione delle \textbf{primary key} e \textbf{foreign key} (da individuare). Specificare inoltre i vincoli di dominio intra-relazionali \textbf{NOT NULL}, \textbf{UNIQUE} e \textbf{CHECK}.

\textbf{Schema relazionale}
\begin{itemize}
    \item \texttt{DIPENDENTE(\underline{Matricola}, Nome, Cognome, DataNascita, Email, Telefono, Indirizzo, Citta, CAP, Stipendio)}
    \item \texttt{DIPARTIMENTO(\underline{Codice}, Nome, Sede, BudgetAnnuale, Telefono, Email,\\ Responsabile, DataCreazione, NumeroDipendenti, Attivo)}
    \item \texttt{PROGETTO(\underline{Codice}, Nome, Descrizione, Budget, DataInizio, DataFine, Stato, Priorita, CodiceDipartimento, Responsabile)}
    \item \texttt{ASSEGNAZIONE(\underline{MatricolaDipendente}, \underline{CodiceProgetto}, OreSettimanali, DataInizio, DataFine, Ruolo, CostoOrario, PercentualeImpiego, Stato, Valutazione)}
\end{itemize}

\textbf{Vincoli di dominio}
\begin{itemize}
\item Email univoca.
\item Stipendio > 1200.
\item Budget >= 0.
\item PercentualeImpiego tra 0 e 100.
\item OreSettimanali > 0.
\item DataFine >= DataInizio.
\end{itemize}
\newpage

\end{enumerate}



\end{document}