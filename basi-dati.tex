...
I DBMS operano su tre livelli, permettendo agli utenti e alle applicazioni che hanno bisogno dei dati, di non doversi preoccupare di come i dati sono memorizzati fisicamente, ma solamente di un modo per dialogare con il DBMS per ottenere quello che vogliono.

\subsubsection*{Livelli di astrazione di un DBMS}
\begin{enumerate}
    ...
    \item \textbf{Livello Logico:} Descrive la struttura dei dati così come la vede il progettista (tabelle, colonne, tipi di dato). 
...
Il livello logico descrive come i dati sono organizzati e strutturati dal punto di vista del progettista, indipendentemente da come vengono memorizzati fisicamente. Include le tabelle, le colonne, i tipi di dati e le relazioni tra le informazioni.  
Il modello più comune è il modello relazionale, che utilizza \textbf{relazioni} (tabelle) per rappresentare le informazioni.

\textbf{Esempio: social network}  
Supponiamo di avere un social network che gestisce utenti, post e amicizie. Nel modello relazionale, potremmo avere queste tabelle principali:
...
\begin{tabular}{|c|c|c|c|}
\hline
\textbf{Codice Post} & \textbf{Codice Utente} & \textbf{Testo} & \textbf{Data} \\
\hline
101 & 1 & "Buongiorno a tutti!" & 2026-01-27 \\
102 & 2 & "Oggi studio informatica" & 2026-01-26 \\
103 & 3 & "Nuovo post sul ciclismo" & 2026-01-25 \\
\hline
\end{tabular}
...
\begin{tabular}{|c|c|}
\hline
\textbf{Codice Utente1} & \textbf{Codice Utente2} \\
\hline
1 & 2 \\
1 & 3 \\
2 & 3 \\
...
\vspace{0.5cm}
\textit{Tabelle: Utenti, Post, Amicizie}
\end{center}

Queste relazioni permettono di rappresentare l’intera struttura logica del social network: chi sono gli utenti, quali post hanno pubblicato e quali relazioni di amicizia esistono tra loro.  


\subsection*{Il linguaggio dei DBMS}
I DBMS forniscono astrazione sui dati, permettendo di lavorare senza preoccuparsi di come le informazioni sono memorizzate fisicamente.  
...
Per progettare una base di dati in modo corretto non è sufficiente conoscere un DBMS o saper scrivere interrogazioni SQL.  
Prima ancora della fase tecnica è necessario costruire un \textbf{modello di dati}, cioè una rappresentazione astratta e formale della realtà che si intende descrivere.

Un modello di dati permette di rispondere a tre domande fondamentali:
\begin{itemize}
\item quali informazioni devono essere memorizzate;
\item quali relazioni esistono tra queste informazioni;
\item quali regole devono essere rispettate affinché i dati siano corretti e coerenti.
\end{itemize}

Senza un modello di dati, la base di dati nasce come un insieme disordinato di tabelle, difficile da comprendere, da mantenere e da estendere nel tempo.

\subsection{Il ruolo dei modelli di dati}

La realtà è complessa, ambigua e ricca di dettagli.  
Un sistema informativo, invece, deve essere preciso, non ambiguo e coerente.

Il modello di dati ha proprio questo ruolo: \textbf{semplificare la realtà senza tradirla}.  
Non tutto ciò che esiste nel mondo reale deve essere rappresentato, ma solo ciò che è rilevante per gli obiettivi del sistema.

Attraverso un modello di dati:
\begin{itemize}
\item si selezionano le informazioni significative;
\item si eliminano ridondanze inutili;
\item si rende esplicita la struttura dei dati;
\item si crea un linguaggio comune tra analisti, progettisti e sviluppatori.
\end{itemize}

\subsection{Livelli di astrazione}

La progettazione di una base di dati avviene su più livelli di astrazione, ognuno con uno scopo preciso.

\paragraph{Livello concettuale}
Descrive la realtà in modo indipendente dalla tecnologia.  
In questo livello si parla di oggetti, relazioni e proprietà, usando un linguaggio comprensibile anche a chi non è un informatico.

\paragraph{Livello logico}
Descrive come i dati sono organizzati secondo un modello specifico, ad esempio il modello relazionale.  
In questo livello compaiono tabelle, attributi e chiavi.

\paragraph{Livello fisico}
Descrive come i dati sono effettivamente memorizzati dal DBMS su disco.  
È il livello più vicino all’hardware ed è dipendente dal DBMS utilizzato.

La separazione tra questi livelli permette di modificare un livello senza dover intervenire sugli altri.

\subsection{Indipendenza dei dati}

Uno dei principali vantaggi dell’uso dei DBMS è l’indipendenza dei dati.  
Questo concetto indica la possibilità di modificare la struttura o l’organizzazione dei dati senza dover riscrivere le applicazioni.

Si distinguono due tipi di indipendenza.

\paragraph{Indipendenza fisica}
Le applicazioni non dipendono da come i dati sono memorizzati fisicamente.  
È possibile cambiare file, indici o modalità di memorizzazione senza modificare i programmi.

\paragraph{Indipendenza logica}
Le applicazioni non dipendono dalla struttura logica della base di dati.  
È possibile aggiungere attributi o tabelle senza compromettere il funzionamento delle applicazioni esistenti.

Il DBMS si occupa di garantire entrambe.

%====================================================================
\section{Il modello relazionale}
%====================================================================

Il modello relazionale è stato introdotto da Edgar Frank Codd nel 1970 ed è tuttora il modello più utilizzato nei sistemi di basi di dati.

La sua forza sta nella semplicità concettuale unita a un forte rigore matematico.

\subsection{Relazioni}

Nel modello relazionale i dati sono rappresentati tramite \textbf{relazioni}, che possono essere visualizzate come tabelle.

Ogni relazione rappresenta un insieme di oggetti omogenei del mondo reale.  
Ad esempio, una relazione \code{Studente} rappresenta l’insieme di tutti gli studenti.

\subsection{Attributi}

Gli attributi rappresentano le proprietà degli oggetti.  
Ogni attributo ha un nome e un dominio di valori ammessi.

La scelta degli attributi è cruciale: attributi inutili rendono la base di dati complessa, attributi mancanti la rendono incompleta.

\subsection{Tuple}

Una tupla rappresenta un singolo oggetto reale.  
Ogni riga della tabella è una tupla distinta.

Ad esempio:
\begin{center}
\code{Studente(Matricola, Nome, Cognome, DataNascita)}
\end{center}

Una tupla valida è:
\begin{center}
\code{(12345, Mario, Rossi, 2006-03-12)}
\end{center}

\subsection{Schema e istanza}

Lo schema descrive la struttura della relazione ed è definito una sola volta.  
L’istanza rappresenta il contenuto effettivo della relazione in un dato momento.

Questa distinzione permette di separare struttura e dati.

%====================================================================
\section{Esempi guidati: dalla realtà alle tabelle}
%====================================================================

Per comprendere realmente il modello relazionale è fondamentale applicarlo a casi concreti.

Consideriamo la progettazione di una base di dati per una scuola.

\subsection{Descrizione informale}

In una scuola esistono studenti che frequentano classi, studiano diverse materie e ricevono valutazioni nel tempo.

Questa descrizione è volutamente informale e incompleta, come accade nella realtà.

\subsection{Individuazione delle entità}

Dalla descrizione emergono chiaramente alcune entità fondamentali:
\begin{itemize}
\item Studente
\item Classe
\item Materia
...
Ogni entità rappresenta un insieme di oggetti con caratteristiche comuni.

\subsection{Scelta degli attributi}

Per ogni entità selezioniamo solo gli attributi realmente necessari.

Ad esempio, per \textbf{Studente} scegliamo una matricola come identificatore univoco, insieme a nome, cognome e data di nascita.

\subsection{Traduzione in relazioni}

Ogni entità viene tradotta in una relazione del modello relazionale.  
Le relazioni risultanti costituiscono lo schema logico della base di dati.

Questo passaggio segna il passaggio dalla comprensione del problema alla sua formalizzazione.
